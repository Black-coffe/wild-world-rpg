...
final class CommunityGuardTest extends CIUnitTestCase
{
    ...
    public function testToleranceIsGroundedInRealWordFormsNotJustSelfConsistentWithTheMap(): void
    {
        ...
        // механизм (стем + допуск-по-хвосту) вообще способен покрыть —
        // «один»/«одна»/«одно» роняют «и» в косвенном падеже («одного», не
        // «одиного» — беглая гласная другого типа, чем «потолок», вне
        // Major 1 story 81), «вдвое»/«втрое»/«быстрее»/«выгоднее»/«всегда»/
        // «никогда»/«достаточно»/«оптимально»/«бесполезно» — неизменяемые
        // наречия/предикативы. Для них якорь — само слово (голая форма):
        // это не проверка склонения (склонять нечего), а страховка от того,
        // что кто-то случайно сломает границу/preg_quote/список целиком.
        $mustMatch = [
            // Числительные с явным падежным чередованием внутри корня
            // (`OBLIQUE_CASE_FORMS`, story 81 Major 1).
            'два'        => 'двух',
            'две'        => 'двух',
            'три'        => 'трёх',
            'четыре'     => 'четырёх',
