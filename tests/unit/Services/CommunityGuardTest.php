...
final class CommunityGuardTest extends CIUnitTestCase
{
    ...
    public function testStopWordDeclensionFormsAreCaughtWithoutReopeningStory69FalsePositives(): void
    {
        $declinedForms = [
            ...
            // Story 81, мелочь №6: твор. падеж мн.ч. — допуск «потолок» расширен 3 → 4.
            'Ограничения задаются разными потолками прочности.',
        ];
    ...
    /**
     * 🔴 Story 81, Major: короткие числительные («два»/«три»/«четыре»/«пять»/
     * «шесть»/«семь»/«восемь»/«девять»/«сто») в косвенном падеже меняют букву
     * ВНУТРИ корня («два» → «двух», «три» → «трёх», «сто» → «ста») — до этой
     * story ни один из них не ловился вовсе (`STEM_OVERRIDES`, машинерия ровно
     * для таких чередований, к числительным не применялась). Ровно тот класс,
     * ради которого рубеж 3 заведён — готовые ответы-оракулы.
     */
    public function testShortNumeralObliqueCaseFormsAreCaught(): void
    {
        $cases = [
            'Хватит трёх заходов на руины.',
            'Собирается из двух частей.',
            'Нужно пяти бойцов для вылазки.',
            'Стоит около ста монет.',
        ];

        foreach ($cases as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isDeny(), "«{$answer}» — косвенный падеж короткого числительного, обязана денится");
            $this->assertSame('lexical_stoplist', $verdict->reason);
        }
    }

    /**
     * 🔴 Story 82, критическое: рубеж обходился тем, что игрок не нажимал
     * «ё» — карта story 81 записана только с «ё» и допуском 0, поэтому
     * «Хватит трех заходов» (обычное для Telegram-набора «е» вместо «ё»)
     * проезжало мимо, пока «Хватит трёх заходов» денилось. Нормализация
     * («ё»/«е» — одна буква при сверке) закрывает это для ЛЮБОГО слова с
     * «ё» в карте, не только перечисленных здесь — сами эти четыре просто
     * покрывают все места, где «ё» встречалась в картах story 81 (`трёх`,
     * `трём`, `нулём` через `OBLIQUE_CASE_FORMS`, стем `перестаёт`).
     */
    public function testYoAndYeAreTreatedAsOneLetter(): void
    {
        $cases = [
            'Хватит трех заходов на руины.',
            'Раздали награду трем игрокам.', // «трем» вместо «трём»
            'Урожай нулем не считается.', // «нулем» вместо «нулём»
            'Прирост перестает расти на этом пределе.', // «перестает» вместо «перестаёт»
        ];

        foreach ($cases as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isDeny(), "«{$answer}» — написание через «е» не должно освобождать от рубежа");
            $this->assertSame('lexical_stoplist', $verdict->reason);
        }
    }

    /**
     * 🔴 Story 82, Major: `## Goal` story 81 обещала, что косвенные падежи
     * коротких числительных перестают проезжать — «Хватит одного захода»
     * (тот же оракул-класс, что «Хватит трёх заходов») и собирательные
     * («Трое суток без воды», «Двое ушли на руины») в это обещание не
     * попали. Покрыты той же техникой `OBLIQUE_CASE_FORMS` — без изменения
     * состава `NUMERAL_WORDS` (это форма сверки, не новое слово в списке).
     */
    public function testOneAndCollectiveNumeralsAreCaught(): void
    {
        $cases = [
            'Хватит одного захода.',
            'Дал одному выжившему.',
            'Трое суток без воды.',
            'Двое ушли на руины.',
            'Пятеро вернулись назад.',
        ];

        foreach ($cases as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isDeny(), "«{$answer}» — «один»/собирательное числительное, обязана денится");
            $this->assertSame('lexical_stoplist', $verdict->reason);
        }
    }

    /**
     * 🔴 Story 82, мелочь: новые формы — принятая цена того же класса, что
     * story 80 (список исключений не заводится), просто не названные в
     * перечне до этой story. «Семью» — винительный падеж СЛОВА «семья», не
     * форма числительного «семь»; «трём» — 1 л. мн.ч. глагола «тереть»;
     * «потолкать» — разговорный глагол, не форма «потолка».
     */
    public function testNumeralCollateralCollisionsAreNamedAndAccepted(): void
    {
        $knownCost = [
            'Он кормит семью на базе.',
            'Мы трём стекло тряпкой.',
            'Потолкать тележку к воротам.',
        ];

        foreach ($knownCost as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isDeny(), "«{$answer}» — известная и принятая цена, не регрессия");
            $this->assertSame('lexical_stoplist', $verdict->reason);
        }
    }

    /**
     * Story 82, ревью седьмого круга: набор обиходных слов, структурно
     * похожих на короткие числительные/«потолок», но не совпадающих с ними
     * ни при какой границе — доказывает, что нормализация «ё»/«е» и новые
     * `OBLIQUE_CASE_FORMS` не расширили покрытие дальше задуманного.
     */
    public function testEverydayWordsResemblingNumeralsStillPass(): void
    {
        $notLeaks = [
            'Возьми стакан воды.',
            'Птичья стая летит на юг.',
            'Сталь идёт на инструменты.',
            'Пора стать сильнее физически.',
            'Стадо бродит у реки.',
            'Это двухдневный переход.',
            'Шестерня в механизме стёрлась.',
            'Пятитонка привезла ресурсы.',
        ];

        foreach ($notLeaks as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isAllow(), "«{$answer}» — обиходное слово, не должно денится стоп-листом");
        }
    }
     ...
     * «-ость», меняющий часть речи, не падежная форма). Story 81 добавляет
     * «перестановка»/«перестройка»/«пороговый» — соседние слова, которые НЕ
     * должны начать денится ни от расширения `упирается`/`окупается`→4, ни от
     * снятия узкой записи `порог`/`перестаёт` (действует формула-3): нужны
     * как явный верхний предел для теста-коллизий («рост коллизий краснеет»,
     * story 81 пункт 5) — если формула однажды расширится ещё, эти три первыми
     * покажут, что расширение зашло дальше, чем задумано.
     */
    public function testReverseFalsePositivesNoLongerMatch(): void
    {
        $notLeaks = [
            ...
            'У нас перестановка мебели.',
            'Идёт перестройка базы.',
    ...
    /**
     * 🔴 Story 81, названная цена расширения (Major 2): снятие узкой записи
     * `'перестаёт' => 2` в story 80 (формула теперь даёт 3 для стема
     * «переста») заодно стало ловить формы ДРУГОГО глагола «переставить»
     * (переместить предмет), не «перестать» (прекратить действие). Цена
     * ПРИНЯТА явно — правило формы важнее контрольного сообщения, тот же
     * принцип story 80 про короткие числительные (список исключений не
     * заводится, он не закрывается).
     */
    public function testPerestaetToleranceWideningHasANamedCost(): void
    {
        $knownCost = [
            'Переставил верстак ближе к воротам.',
            'Игрок переставит маяк.',
        ];

        foreach ($knownCost as $answer) {
            $verdict = $this->guard()->verdict($answer, 'Расскажи про механику.', null);
            $this->assertTrue($verdict->isDeny(), "«{$answer}» — известная и принятая цена расширения допуска «переста», не регрессия");
            $this->assertSame('lexical_stoplist', $verdict->reason);
        }
    }

    /**
     * 🔴 Story 81, названная цена сужения (Major 2, симметрично предыдущему):
     * `'достаточно' => 1` (story 79, защищает «достаточность») тем же движением
     * перестаёт ловить «достаточного»/«достаточной» — падежные формы
     * согласованного прилагательного «достаточный». Направление безопасное
     * (ошибка — allow контрольной фразы, не пропуск утечки формы числа), но
     * цена реальна и названа, а не объявлена нулевой.
     */
    public function testDostatochnoToleranceNarrowingHasANamedCost(): void
    {
        $verdict = $this->guard()->verdict('Достаточного запаса еды хватит на вылазку.', 'Расскажи про механику.', null);
        $this->assertTrue($verdict->isAllow(), 'известная и принятая цена сужения допуска «достаточно» — падежная форма «достаточного» проходит, не регрессия');
    }

    /**
     * 🔴 Story 81, мелочь №6, вторая половина: аспектная пара «окупается»
     * (несов. вид) / «окупились» (сов. вид, «окупиться») — другая словарная
     * форма глагола, не падежное чередование (стем расходится на пятой
     * букве: «окупа-» vs «окупи-»). НЕ чинится точечно — глагольные видовые
     * пары в русском многочисленны, закрывать их по одной означает открыть
     * тот же нескончаемый список, что уже дважды подводил (story 78/79).
     * Названная, принятая, непроверенная-и-забытая цена — не регрессия.
     */
    public function testOkupilisPerfectiveAspectFormIsAcceptedGap(): void
    {
        $verdict = $this->guard()->verdict('Вложения окупились быстро.', 'Расскажи про механику.', null);
        $this->assertTrue($verdict->isAllow(), 'известный и принятый пробел — аспектная пара «окупиться» не покрыта, не регрессия');
    }
    ...
    public function testToleranceIsGroundedInRealWordFormsNotJustSelfConsistentWithTheMap(): void
    {
        ...
        // 🔴 Story 81, пункт 2: якорь — настоящая словоформа — обязан быть у
        // КАЖДОГО слова обоих списков (кроме substring-фрагментов «дцать»/
        // «десят»/«сотн» — они не сверяются границей слова вовсе), не только
        // у семи, что были здесь после story 80. Без анкера сужающая запись
        // карты у любого из непроверенных слов проходит мимо этого теста
        // молча — ровно тот класс, что нашёл шестой круг ревью.
        //
        // Часть слов не имеет продуктивного словоизменения, которое текущий
        // механизм вообще способен покрыть — «вдвое»/«втрое»/«быстрее»/
        // «выгоднее»/«всегда»/«никогда»/«достаточно»/«оптимально»/
        // «бесполезно» — неизменяемые наречия/предикативы. Для них якорь —
        // само слово (голая форма): это не проверка склонения (склонять
        // нечего), а страховка от того, что кто-то случайно сломает
        // границу/preg_quote/список целиком.
        $mustMatch = [
            // Числительные с явным падежным чередованием внутри корня
            // (`OBLIQUE_CASE_FORMS`, story 81 Major 1) + собирательные
            // (story 82 Major).
            'два'        => 'двоих',
            'две'        => 'двух',
            'три'        => 'трое',
            ...
            'сто'        => 'ста',
            'ноль'       => 'нуля',
            // «Один»/«одна»/«одно» — беглая «и» (story 82 Major).
            'один'       => 'одного',
            'одна'       => 'одной',
            'одно'       => 'одному',
            // Числительные, которые формула по хвосту уже покрывала (story 79).
            'сорок'      => 'сорока',
            'тысяч'      => 'тысячами',
            'миллион'    => 'миллионов',
            'процент'    => 'процентах',
            // Глаголы с чередованием гласной перед окончанием (story 78/81).
            'упирается'  => 'упираются',
            'окупается'  => 'окупаются',
            'перестаёт'  => 'перестанет',
            // Существительное с беглой гласной (story 73/81).
            'потолок'    => 'потолками',
            'порог'      => 'порогами',
            // Неизменяемые слова — якорь-самослово (страховка от поломки
            // границы/списка, не проверка склонения — склонять нечего).
            'вдвое'      => 'вдвое',
            'втрое'      => 'втрое',
            'быстрее'    => 'быстрее',
            'выгоднее'   => 'выгоднее',
        ...
        $numeralWords = $ref->getReflectionConstant('NUMERAL_WORDS')->getValue();
        $lexicalWords = $ref->getReflectionConstant('LEXICAL_STOPLIST')->getValue();
        $fragments    = $ref->getReflectionConstant('SUBSTRING_FRAGMENT_WORDS')->getValue();
        $expectedWords = array_values(array_diff(array_unique(array_merge($numeralWords, $lexicalWords)), $fragments));
        sort($expectedWords);
        $anchoredWords = array_keys($mustMatch);
        sort($anchoredWords);
        $this->assertSame(
            $expectedWords,
            $anchoredWords,
            'якорь обязан существовать у КАЖДОГО слова обоих списков (кроме substring-фрагментов) — список якорей разошёлся со списками стоп-слов',
        );
