...
- **V15** ✅ **SHIPPED v0.51.244 (2026-05-25) — 🏁 ЗАКРЫТИЕ ФАЗЫ 3.** Endgame polish (честный closer, user pick). **Audit-first вскрыл drift:** literal-scope «T4 images» уже закрыт V14 image-tail (v0.51.243) → re-scope в polish + закрытие. **Аудит эндгейм-блока:** все 4 фракции укомплектованы (объект → 3-шаговая цепочка → signature оружие+броня), меню/маршруты/гейты/картинки на месте; нашлись 3 реальных хвоста: (1) **канон-дрейф** — GAME_DESCRIPTION упоминал только фракц. оружие, не броню; (2) **gear-detail картинка** — owned фракц. броня в осмотре падала на `default_armor.jpg` (`GearArmorDetailAction` держит карту только для standard/, V14-картинки в professional/); (3) **полу-честность** — `physical/fire/poison_resistance` + `speed/stealth_modifier` читаются ТОЛЬКО в PvP (`PvpDamageCalculator`), PvE (`EquipmentService`) учитывает лишь `armor_value`, а игроку эти числа вообще не показывались (`special_bonus` — dead column: не отображается, в PvE не работает). **Фиксы (pure display + docs, 0 миграций, 0 бизнес-логики):** новый pure-хелпер `App\Services\Display\OutfitDisplayHelper` (резолв professional/-картинки фракц. брони + рендер ненулевых сопротивлений + честная PvP-сноска); проброшен в `GearArmorDetailAction` (картинка + блок «Спец-свойства» + сноска) и `ArmorRecipePreviewT3Action` (блок + сноска перед крафтом); GAME_DESCRIPTION += фракц. броня (4 фракции, полный лут оружие+броня, честная PvP-оговорка). Guarded non-zero → базовая броня не затронута; заодно честно раскрывает сопротивления у S18-Legendary. **+7 тестов (OutfitDisplayHelperTest, 789→796), phpstan L9 ✅, composer test 796/796.** Картинки фазы — уже отгружены (V14 tail). Tier-3 real-game render-smoke (caption/photo).
