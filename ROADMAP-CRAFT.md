...
| S26 | Defensive structures (walls/fence) — старт Фазы 6 🟠 | ✅ shipped 🔀 | v0.51.207 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | 🔀 **Audit-drift**: PvP = полевые дуэли (НЕ осада базы); round loop под RNG-fence (только детерминированные эффекты); `building_type`=MySQL ENUM (ALTER). User pick: WoodenWall+BarbedFence, эффект ТОЛЬКО когда защитник на своей клетке со структурами, GameSettings+cap 40%, WatchTower→S26b. **ADR-030** (fairness: at-base/cap/decay+tax). Shipped: 2 migrations (ALTER 2 ENUM +defensive; 2 buildings+2 tasks; 4 GameSettings combat-knobs) + Config\Buildings 2 entries + BuildListAction 2 кнопки (reuse generic build→completion→character_buildings defensive) + `DefenseStructureService` (getDefenseProfile at-cell/cap + applyDecay) + `PvpRoundOrchestrator::simulateFight(+?defense=null)` (wall reduction + fence damage, детерминированно) + AttackPlayerAction wiring. +6 tests (671→677), phpstan L9 ✅. **Fixture-fence regression passed** (defense=null no-op). **Testbot e2e ✅**: build chain → profile 0.15/3 → слабый защитник (lvl1) победил сильного атакующего (lvl305) у базы через макс wall+fence → decay 200→190. |
| S26b | WatchTower — alert-range detection + defender initiative 🟠 | ✅ shipped | v0.51.208 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | Завершает defensive-трио (стена/ограда/вышка), закрывает П4. **ADR-031.** Audit: WatchTower = genuine net-new (0 drift); `determineInitiative` чистая → initiative RNG-fence-safe; alert = отдельная подсистема (movement-hook, не round loop). Shipped: 2 migrations (WatchTower building hp300/tax700 + buildWatchTower task; 3 GameSettings combat-knobs `defense.tower.alert_range_cells=5`/`defender_initiative_bonus_percent=8`/`alert_cooldown_sec=1800`) + Config\Buildings entry + BuildListAction кнопка (reuse generic build, ENUM уже defensive) + `DefenseStructureService` +initiative_bonus (presence-based, не стакает) + `PvpRoundOrchestrator::determineInitiative(+?defense=null)` (множит i-score owner'а, детерминированно) + **`TowerAlertService`** (notifyTowersNear: Евклидов радиус, exclude own, distinct-once, cache-кулдаун) + hook в MarchingTaskHandler. Fairness (ADR-031): initiative вне cap 40% (осознанно — мягкий 8%, presence-based, at-base+decay+tax 700). +15 tests (677→692), phpstan L9 ✅. **Fixture-fence regression passed** (defense=null identical). **Testbot e2e ✅**. |
| S27 | Craft queue UI visualization 🟡 | ✅ shipped | v0.51.209 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | Audit: spec ТОЧЕН (no drift, как S26b). Очередь крафта (v0.51.129) живая — `queued`-tasks (slot cap 3 / per-recipe cap 10, ресурсы upfront, FIFO per-recipe dequeue, CancelQueuedCraftAction refund). Реальный gap = нет consolidated view (был только per-creation notify). Pure UI (без migration/GameSettings). Shipped: `ActiveTasksService::getCraftQueue()` (active+queued craft tasks по `tasks.type='craft'`, ETA/qty/FIFO, тестируемо) + `ShowCraftQueueAction` (callback `craftQueue`: экран «сейчас крафтится»+ETA / «в очереди»+❌cancel-кнопки reuse `cancelQueued_`; edit-in-place; пустое состояние) + entry-кнопки «📋 Очередь крафта» (queued-notify + 3 верстака + cancel-success) + CallbackRoutes. +5 tests (692→697), phpstan L9 ✅. **Testbot e2e ✅**. |
| S28 | Seasonal craft rotation 🟠 | ✅ shipped | v0.51.210 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | Самая крупная сессия. **ADR-032.** Audit: «season» уже занято endgame `SeasonResetService` → namespace `seasonal.*` отдельно; genuine net-new framework; recipe-gating reusable; эффекты на существующие системы (S19 heal, без новых «температурных» механик). Season = детерминир. math (anchor+cycle, без DB-состояния); anchor=2026-05-20 → Зима активна на запуске (живой pilot, не BUILT-BUT-DEAD). Shipped: 3 migrations (5 `seasonal.*` GameSettings category=world + 5 winter crafted_items(drug)+5 tasks + 10 `medical.*` heal category=combat) + `SeasonalCraftService` (season math + isSeasonActive + SEASON_RECIPES) + `required_season` gate в GenericCraftActionStart (зеркало required_quest) + **5 winter рецептов** (Зима выживания: отвар/сбитень/мазь/похлёбка/заготовки, warming consumables, heal через UsePharmacyAction) + `SeasonalCraftSelect` (callback `seasonalCraft`, динамич. по сезону) + `SeasonalRecipePreviewAction` (prefix `craftPreviewSeasonal_`) + entry-кнопка в GeneralCrafting + `SeasonalTransitionHandler` (daily 00:00 broadcast смены, cache-state, first-run silent → анонс только на реальной смене) + 5 images (loot.medicine V4). FOMO-mitigations (ADR-032): возврат каждый год / 21д / killswitch `seasonal.enabled` / изготовленное не исчезает. +7 tests (697→704), phpstan L9 ✅. **Testbot e2e ✅**. |
| S29 | Content-pass: lying descriptions cleanup 🟢 | ✅ shipped | v0.51.211 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | Audit (workflow `feedback_content_pass_workflow`): прогон user-facing строк (crafted_items / events / buildings) vs реальные механики. **Чисто почти везде** (consume-claims честны: EmergencyTransfusion dur=1 «одноразовый», SurgicalKit dur=3 «×3»; MeteorShelter/MeteorImpact точны после v0.51.139). Единственный документированный mismatch (S19) — **disease-события**: «Жаркая лихорадка» врала про персистентное состояние «заболел» (которого в коде НЕТ — только урон по тикам); «Эпидемия» + «Лихорадка» умалчивали реальные рычаги (Antiseptic −50%, на базе риск ~8× ниже: base_idle 0.06 vs biome_idle 0.5), хотя sibling «Метеоритный удар» их называет (gold-standard). Fix: idempotent UPDATE-миграция 2 event descriptions (WHERE old-text → idempotent; источника в коде/сидерах нет — правка данных) + GAME_DESCRIPTION «Биологические угрозы» 2 строки в соответствие. Без новых механик/тестов (content-data; smoke = prod SQL SELECT). phpstan L9 ✅, tests 704. **Prod SQL verify ✅**. |
| S30 | Admin tools + ROADMAP-vNext 🟢 — 🏁 ФИНАЛ 30 сессий | ✅ shipped | v0.51.212 | 2026-05-20 | [`daily/2026-05-20`](file:///C:/Projects/mmorpg-vault/daily/2026-05-20.md) | 🏁 **Закрывает ROADMAP-CRAFT v1 (30/30).** Audit: `/admin/craft-tree` live (v0.51.180), `workbenchTier` T1/T2/T3 уже есть. Shipped: **admin CSV-экспорт** craft-tree (`/admin/craft-tree/export` → `CraftTreeService::buildCsvRows()` рецепты+постройки, UTF-8 BOM для Excel/кириллицы; кнопка в view) + `defensive` building-icon fix + **`ROADMAP-CRAFT-vNext.md`** (5-осевой алгоритм прогнан по 14 кандидатам + open-tails v1 → top-ранжирование → 6 фаз × 5 сессий = 30) + **retrospective** (`inbox/2026-05-20-roadmap-v1-retrospective.md`: 23/25 drift, GameSettings ROI, reuse-паттерны, PvP RNG-fence, процессные уроки). Season-filter/mass last_reviewed-bump — намеренно отложены (низкая ценность). phpstan L9 ✅, tests 704. **Testbot smoke ✅** (buildCsvRows + curl export). |
...
- 🏁 **Фаза 6 (S26-S30) ЗАКРЫТА** — Polish + ROADMAP-vNext. **S26** defensive structures (стена/ограда) + **S26b** WatchTower (alert+initiative) → defensive-трио; **S27** craft-queue UI; **S28** seasonal craft rotation (зимний pilot); **S29** content-pass (disease-события); **S30** admin CSV-экспорт + `ROADMAP-CRAFT-vNext.md` + retrospective. Closed v0.51.212.

---

## 🏁 ROADMAP-CRAFT v1 ЗАКРЫТ (30/30 сессий, 2026-05-17 → 2026-05-20)

Все 6 фаз закрыты, прод-теги v0.51.183 → v0.51.212. Тесты 555→704, phpstan L9 держался все 30 сессий, 0 прод-инцидентов от роудмеп-кода. Следующие 30 сессий — [`ROADMAP-CRAFT-vNext.md`](./ROADMAP-CRAFT-vNext.md) (синтезирован 5-осевым алгоритмом в S30). Ретроспектива — `mmorpg-vault/inbox/2026-05-20-roadmap-v1-retrospective.md`. **Анонс игрокам всей Фазы 6 + эндгейма — батчем (после финала).**
