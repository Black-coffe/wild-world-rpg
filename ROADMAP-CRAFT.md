...
- ⏳ **Фаза 4 (S16-S20)** — T3 expansion. **S16 ✅** v0.51.198 (Professional Workbench + ADR-026 reusable pattern). **S17 ✅** v0.51.199 (5 T3 weapons + schema `required_crafted_items`). **S18 ✅** v0.51.200 (4 T3 armor + new `output_type='outfit'` dispatch path, reusable для T2 armor refactor). S19-S20 pending (6 T3 рецептов: medical / utility).
