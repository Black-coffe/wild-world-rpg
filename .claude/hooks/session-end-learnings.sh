...
# transcript via a one-shot headless call on the junior rung (opt-in - it spends tokens;
# sonnet until a Haiku 5 exists, ADR-007).
set -uo pipefail
...
if [ "${VULYK_AUTOLEARN:-0}" = "1" ] && [ -n "$transcript" ] && [ -f "$transcript" ] && command -v claude >/dev/null 2>&1; then
  tail -c 200000 "$transcript" | claude -p --model sonnet \
