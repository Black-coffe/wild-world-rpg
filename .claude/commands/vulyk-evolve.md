...
4. Propose. On a branch `vulyk/evolve-<date>`, write the actual diffs: edits to the constitution (`CLAUDE.vulyk.md` if it exists, else `CLAUDE.md`) - keep it lean, it rides every agent that loads it, `.claude/rules/`, agent prompts, new skill scaffolds; move archived skills to `.claude/skills/_graveyard/<name>/` adding `RETIRED.md` (date, reason, evidence). Append a CHANGELOG.md entry justifying every change, one line each.
