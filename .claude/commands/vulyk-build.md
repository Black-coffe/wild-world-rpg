...
2. **The fallback loop.** Repeat until a terminal `next`:
   ...
   | `build:<wave>` | `bash scripts/wave-check.sh docs/specs/<slug>` first - a collision here is a plan defect, fix the story files before dispatching anything. Then dispatch every entry of the status object's `wave_stories` - each already `{"file","story","worker","model","repeat"}` (C3 - the driver never opens a story file to learn its worker or model) - to the named `worker` (`worker-code`/`worker-test`) with `model: <model>` on the first dispatch and `model: <top_model>` on a story's second dispatch (ADR-012: the gate model takes the retry), one message, cap 4 concurrent, each worker getting exactly its story file, its map slice pointer and the relevant `.claude/rules/` paths. A dispatch that errors out (a tool error, an abort) is `worker threw: <message>`; an empty final
