...
⚔️ ударить первым), атакующий — экран ожидания с обратным отсчётом. Точки входа:
`AttackPlayerAction` (гейт `resolveStandoffOutcome()`, до анти-спам-кулдауна), три
`Actions/PVP/Standoff{Check,Leave,Hold}Action`, `RunAwayAction` (закрывает окно как `fled`),
task-handler `App\TaskHandlers\PVP\StandoffExpiryHandler` (крон, ленивое истечение). Сердце —
`App\Services\PVE\PvpStandoffService` + `StandoffNotifier`, модель `PvpStandoffModel`
(`pvp_standoffs`).
