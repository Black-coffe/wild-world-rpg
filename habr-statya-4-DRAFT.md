...
Картинки (генерить отдельно, стиль «найденная фотоплёнка», 0 текста на картинке):
  cover — выживший один у пыльного терминала ночью, на экране строка кода, постапок-палитра (КДПВ)
  meme1 — рубильник «OFF», но лампочка горит (булев false, который true)
  meme2 — форма с данными, на выходе пустой конверт (allowedFields глотает поле)
  meme3 — строгий вахтёр заворачивает новичка без пропуска (ENUM + STRICT)
  meme4 — механик чинит мотор машины на ходу, пассажиры даже не заметили (Entity+ArrayAccess)
  meme5 — эмодзи 🔥 проходит через узкую дверь и выходит знаком «?» (utf8mb3)
============================================================
-->
