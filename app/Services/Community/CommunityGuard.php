...
final class CommunityGuard
{
     ...
     *
     * 🔴 Story 80 — решение по коротким числительным, зафиксированное явно.
     * Короткие корни (≤4 буквы: «три», «семь», «сорок» и другие) на допуске
     * формулы (1 буква) ловят обиходные слова той же длины: «семь» → «семья»,
     * «три» → «трио», «сорок» → «сорока» (птица). Цена ПРИНЯТА, не устранена
     * точечным списком исключений — по тому же принципу, что уже дважды не
     * сработал в этой истории (story 78 сузила покрытие плоским default,
     * story 79 молча потеряла числительные без записей в карте): список
     * слов-исключений в русском не закрывается, а на СПИСКЕ СТОП-СЛОВ борьба
     * с обходом важнее борьбы с ложным отказом контрольного сообщения — цена
     * ошибки денить лишнего «Семья выживших держит базу вместе.» на порядок
     * ниже цены пропустить реальную утечку через то же короткое слово с
     * лишней буквой. Названный, измеренный, принятый костыль — не регрессия;
     * см. `testShortNumeralWordsKnownCollisionsAreAcceptedCost()`.
     *
     * 🔴 Story 82 — та же цена, две новые формы, дописанные в перечень явно
     * (не новые находки-сюрпризы, а забытые записи того же принятого класса):
     *  - «семь» → «семью» (винительный падеж СЛОВА «семья», не форма
     *    числительного «семь») — «Он кормит семью на базе.» денится.
     *  - `OBLIQUE_CASE_FORMS['три']` → «трём» совпадает с глаголом «тереть»
     *    1-го лица мн.ч. наст. вр. — «Мы трём стекло тряпкой.» денится.
     * См. `testNumeralCollateralCollisionsAreNamedAndAccepted()`.
     */
     ...
     *
     * 🔴 Story 81 — известный НЕисправленный предел этого приёма: аспектная
     * пара «окупается» (несов. вид) / «окупились» (сов. вид, «окупиться») —
     * не падежное чередование, а другая словарная форма глагола (другой
     * инфинитив), стем расходится на пятой букве («окупа-» vs «окупи-»).
     * Не чинится тут же — глагольные видовые пары в русском многочисленны,
     * закрывать их точечно означает открыть тот же нескончаемый список,
     * что уже дважды подводил (story 78/79); «окупились» остаётся ПРИНЯТОЙ
     * ценой, названной явно (`## Findings` story 81), а не тихой дырой.
     */
    ...
    /**
     * 🔴 Story 81, Major — короткие числительные («два», «три», «четыре»,
     * «пять», «шесть», «семь», «восемь», «девять», «сто», «ноль») меняют
     * букву ВНУТРИ корня в косвенном падеже («два» → «двух», «три» → «трёх»,
     * «сто» → «ста»), не только добавляют буквы в хвост — `STEM_OVERRIDES`
     * (стем + допуск-по-формуле на его хвост) для таких чередований не
     * годится, потому что расхождение не в хвосте, а внутри слова, причём
     * КАЖДЫЙ косвенный падеж — отдельная, не выводимая из стема, форма
     * («двух»/«двум»/«двумя» — три разные буквы после «дв», не одна общая
     * подстрока с допуском). Поэтому — не стем+допуск, а список ПОЛНЫХ
     * альтернативных форм слова, каждая сверяется границей слова и допуском
     * 0 (это уже готовые, полностью просклонённые формы — суффикс от них
     * ловить нечего, ADR не просит покрывать составные конструкции вроде
     * «двухсот»). Длинные числительные («процент», «тысяч», «миллион»)
     * формула по хвосту уже покрывает — сюда не добавлены.
     *
     * 🔴 Story 82, Major — «один»/«одна»/«одно» и собирательные числительные
     * («двое», «трое»…) добавлены той же техникой, не новыми записями в
     * `NUMERAL_WORDS` (список остаётся неизменным — это форма СВЕРКИ, не
     * добавление слова). «Один» роняет «и» в косвенном падеже иначе, чем
     * «потолок» роняет «о» («одного», не «одиного» — беглая гласная другого
     * типа), «одна» меняет последнюю букву корня перед падежным окончанием
     * («одной», не «однаой»). Собирательные («трое», «пятеро»…) — отдельная
     * лексема с тем же числовым смыслом, что и количественное числительное
     * (как «сотня» при «сто», story 73) — покрыты именительным падежом
     * собирательного и падежом «-их» (родительный/винительный одушевл.,
     * самый частый в разговорной речи: «Двое ушли», «Хватило троих»).
     * Дательный/творительный собирательных («двоим», «двоими»…) НЕ покрыты —
     * тот же принцип, что и «окупились» (story 81): дальше вглубь падежной
     * парадигмы редких форм — открывать список, который не закрывается, а не
     * закрывать конкретную дыру. Названная, принятая цена, не тихая дыра.
     */
    private const OBLIQUE_CASE_FORMS = [
        'два'    => ['двух', 'двум', 'двумя', 'двое', 'двоих'],
        'две'    => ['двух', 'двум', 'двумя', 'двое', 'двоих'],
        'три'    => ['трёх', 'трём', 'тремя', 'трое', 'троих'],
        'четыре' => ['четырёх', 'четырём', 'четырьмя', 'четверо', 'четверых'],
        'пять'   => ['пяти', 'пятью', 'пятеро', 'пятерых'],
        'шесть'  => ['шести', 'шестью', 'шестеро', 'шестерых'],
        'семь'   => ['семи', 'семью', 'семеро', 'семерых'],
        'восемь' => ['восьми', 'восьмью', 'восьмеро', 'восьмерых'],
        'девять' => ['девяти', 'девятью', 'девятеро', 'девятерых'],
        'сто'    => ['ста', 'стам', 'стами', 'стах'],
        'ноль'   => ['нуля', 'нулю', 'нулём', 'нуле'],
        // «Один»/«одна»/«одно» — местоименное числительное, оба рода теряют
        // «и» в косвенном падеже мужского/среднего рода одинаково («одного»
        // общее и для «один», и для «одно»), «одна» меняет последнюю букву
        // корня. Story 82.
        'один'   => ['одного', 'одному', 'одним', 'одном'],
        'одна'   => ['одной', 'одну'],
        'одно'   => ['одного', 'одному', 'одним', 'одном'],
    ];

    /**
     * Story 78/79/80 — допуск на лишние буквы-окончания. Default читается из
     * ДЛИНЫ стема (story 73's формула: ≤4 буквы → допуск 1, ≥5 букв → допуск
     * 3) — story 79 нашла, что story 78's плоский default=1 молча снял с
     * этой формулы длинные числительные («процент», «тысяч», «миллион»,
     * «четыре», «шесть», «восемь», «девять», «сорок», «вдвое», «втрое»),
     * которые никогда не получали собственной записи в карте именно потому,
     * что формула их и так покрывала.
     *
     * 🔴 Story 80 — правило записи в этой карте: **любая запись УЖЕ формулы
     * обязана нести в комментарии конкретное постороннее слово**, которое
     * иначе поймается на допуске по формуле, — не абстрактное «на всякий
     * случай». Ревью пятого круга нашло две записи без такого слова
     * (`'порог' => 2`, `'перестаёт' => 2`), сужавшие формулу (которая дала бы
     * 3) без причины и пропускавшие мимо рубежа «перестанет»/«перестанут» —
     * обе удалены, действует формула.
     *
     * 🔴 Story 81 — расширение (запись ШИРЕ формулы, как и снятие узкой
     * записи, отдающее слово формуле) НЕ бесплатно: у него своя, отдельная
     * цена, и она названа явно, а не объявлена нулевой. Снятие узкой записи
     * `'перестаёт' => 2` (формула теперь даёт 3 для стема «переста») заодно
     * стало ловить «Переставил верстак ближе к воротам»/«Игрок переставит
     * маяк» — формы ДРУГОГО глагола «переставить» (переместить предмет), не
     * «перестать» (прекратить действие); совпадение стема «переста» у двух
     * разных глаголов — случайность русской морфологии, не ошибка выбора
     * допуска. Цена ПРИНЯТА (правило формы важнее контрольного сообщения,
     * тот же принцип story 80 про короткие числительные), не отрицается —
     * см. `testPerestaetToleranceWideningHasANamedCost` и `## Findings`
     * story 81.
     *
     * 🔴 Story 80 — класс «-ость» (суффикс, меняющий часть речи, не падежную
     * форму) решён ТОЛЬКО для «достаточно», намеренно, не наполовину.
     * «Оптимально»/«бесполезно» делят корень со списком R2
     * (`COMPARATIVE_EVALUATION_ROOTS`: `оптимальн`, `полезн`) — «оптимальность»
     * и «бесполезность» содержат этот корень БУКВАЛЬНО, и добавление им сюда
     * допуска 1 (по образцу «достаточно») НЕ МЕНЯЕТ итоговый вердикт: без
     * записи формула (3) ловит их лексикой (рубеж 3, `lexical_stoplist`) —
     * с записью-в-1 их вместо этого ловит рубеж формы (R2, `comparative_claim`,
     * рубеж 1, идёт следом за рубежом 3). Deny в обоих случаях — проверено
     * прогоном обоих вариантов (`## Findings` story 80). Запись была бы
     * мёртвым кодом, создающим ложное впечатление, что именно она защищает.
     * 🔴 Story 81 — уточнение условия: «deny в обоих случаях» верно ПРИ
     * `community.guard.comparative_form !== 'off'` (default `'deny'`, т.е.
     * рубеж формы включён) — это условие ветки в `verdict()`, не в этой
     * карте. При `comparative_form=off` рубеж формы не считается вовсе, и
     * без записи здесь «оптимальность»/«бесполезность» всё ещё денятся
     * лексикой (рубеж 3 не зависит от этого килсвитча) — вывод не меняется
     * практически, но условие называется явно, а не подразумевается молча.
     * «Достаточно» — единственное слово списка, чей корень НЕ входит в R2,
     * поэтому единственное, для кого этот допуск реально решает исход
     * (allow vs deny, а не только КАКИМ рубежом denится).
     * Зафиксировано тестами `testReverseFalsePositivesNoLongerMatch` (allow
     * для «достаточность») и
     * `testOstSuffixWordsSharingComparativeRootsStillDenyEitherWay`
     * (deny для двух других независимо от того, есть тут запись или нет).
     */
    private const TOLERANCE_OVERRIDES = [
        // Уже формулы (1): «стол»/«сток» — посторонние слова, story 78.
        'сто'        => 0,
        // Уже формулы (3): «достаточность» — другая часть речи через
        // суффикс «-ость», не падежная форма, story 79.
        //
        // 🔴 Story 81, названная цена сужения: тот же допуск-1 не пускает и
        // «достаточного»/«достаточной» (падежные формы согласованного
        // прилагательного «достаточный» — «Достаточного запаса еды хватит на
        // вылазку» проходит рубеж). Направление безопасное (allow — не
        // регрессия сама по себе), но это реальная, а не нулевая цена
        // сужения — принятая, не проверенная-и-забытая (`## Findings`).
        'достаточно' => 1,
        // Шире формулы (3 → 4): «упираются»/«окупаются» — спрягаемая форма
        // 3-го лица множественного числа меняет гласную ПЕРЕД окончанием,
        // формула по длине стема их не покрывает, story 78.
        'упирается'  => 4,
        'окупается'  => 4,
        // Шире формулы (3 → 4): «потолками» (твор. падеж мн.ч. от «потолок»
        // через стем «потол») — формула по длине стема даёт только 3,
        // story 81. 🔴 Story 82, названная цена расширения: тот же допуск-4
        // ловит «Потолкать тележку к воротам.» — разговорный глагол
        // «потолкать» (несколько раз толкнуть), не форма «потолка». Цена
        // принята явно, тот же принцип, что для «переставил»/«окупились».
        'потолок'    => 4,
    ];
     ...
     *
     * 🔴 Story 79 — default допуска ОБЯЗАН читаться из формулы по длине стема
     * (см. докблок `TOLERANCE_OVERRIDES`), не из плоского числа: плоский
     * default=1 (story 78) молча снял числительные с их падежных форм
     * («процентов», «тысячами», «миллионов» перестали денится), потому что
     * они не имели собственной записи в карте — карта покрывает только
     * ИСКЛЮЧЕНИЯ из формулы, не заменяет её собой.
     */
    /**
     * 🔴 Story 82, критическое — «ё»/«е» нормализуются к одной букве ОБЕИМИ
     * сторонами сверки (входящий текст И карта), не парными записями:
     * `'трёх' => 'трех'` для каждого слова с «ё» удвоило бы карту и однажды
     * забыло бы пару (ровно тот класс дефекта, что уже дважды съедал
     * покрытие молча — story 78/79). В русском Telegram-наборе «ё» — не
     * нажимаемое меньшинство (не в основной раскладке), «Хватит трех
     * заходов» (без «ё») был живым обходом рубежа поверх «Хватит трёх».
     */
    private function matchesStopWord(string $lower, string $word): bool
    {
        $lower = self::normalizeYo($lower);

        if (in_array($word, self::SUBSTRING_FRAGMENT_WORDS, true)) {
            return str_contains($lower, self::normalizeYo($word));
        }

        $stem      = self::normalizeYo(self::STEM_OVERRIDES[$word] ?? $word);
        $tolerance = self::TOLERANCE_OVERRIDES[$word] ?? $this->defaultToleranceFor($stem);
        ...
        // Story 81 — косвенные падежи коротких числительных: полные готовые
        // формы, допуск 0 (см. докблок `OBLIQUE_CASE_FORMS`).
        foreach (self::OBLIQUE_CASE_FORMS[$word] ?? [] as $obliqueForm) {
            if (preg_match(
                '/(?<![\p{L}])' . preg_quote(self::normalizeYo($obliqueForm), '/') . '(?![\p{L}])/u',
                $lower,
            ) === 1) {
                return true;
            }
    ...
    /**
     * Story 73's формула, вынесенная в отдельный метод story 79: короткие
     * стемы (≤4 буквы) получают допуск 1 (консервативно — story 69 доказала,
     * что бОльший допуск на коротких корнях ловит случайные посторонние
     * слова), длинные (≥5) — 3 (достаточно для большинства русских падежных
     * окончаний, недостаточно только там, где `TOLERANCE_OVERRIDES` явно
     * переопределяет).
     */
    private function defaultToleranceFor(string $stem): int
    {
        return mb_strlen($stem) <= 4 ? 1 : 3;
    }

    /**
     * Story 82 — «ё» и «е» как одна буква при сверке стоп-слов (см. докблок
     * `matchesStopWord()`). НЕ используется вне сверки стоп-слов — рубежи
     * 1/2/4/5 (форма, вопрос, стоп-темы, dormant) её не используют, им это
     * не нужно и не спрашивалось (Non-goals story 82: не трогать правила
     * формы и остальные рубежи).
     */
    private static function normalizeYo(string $s): string
    {
