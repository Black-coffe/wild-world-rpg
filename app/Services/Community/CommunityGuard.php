...
final class CommunityGuard
{
    ...
    /**
     * Story 78/79 — допуск на лишние буквы-окончания. Default читается из
     ...
     * что формула их и так покрывала — реестр `TOLERANCE_OVERRIDES` нужен
     * ТОЛЬКО для слов, где формула ошибается в любую сторону:
     *  - недостаточно (глаголы с внутренним чередованием — `упирается`/
     *    `окупается` требуют 4, формула по длине стема даёт 3);
     *  - избыточно (формула по длине дала бы «достаточно»(10 букв, ≥5→3)
     *    допуск 3, а трёх букв хватает ровно на «-сть» — суффикс, меняющий
     *    часть речи на «достаточность», не падежную форму).
     * Слово без записи в обеих картах получает СТЕМ = само слово и ДОПУСК
     * по формуле от длины этого стема.
     */
    private const TOLERANCE_OVERRIDES = [
        // «сто» практически не склоняется в разговорном употреблении; допуск
        // по формуле (3 буквы → 1) ловил «стол»/«сток» — посторонние слова,
        // story 78.
        'сто'        => 0,
        // «достаточно» — предикатив/наречие, не склоняется; формула по длине
        // (10 букв → 3) поймала бы «достаточность» (другая часть речи через
        // суффикс «-ость», не падежная форма) — story 79.
        'достаточно' => 1,
        'порог'      => 2, // порог/порога/пороги/порогов
        'потолок'    => 3, // от стема «потол»: -ок/-ка/-ки/-ков
        'упирается'  => 4, // от стема «упира»: -ется/-ются (формула даёт лишь 3)
        'окупается'  => 4, // от стема «окупа»: -ется/-ются (формула даёт лишь 3)
