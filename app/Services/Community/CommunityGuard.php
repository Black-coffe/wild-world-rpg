 ...
 *     ADR-178 поправка №2)/клауза-императив+маркер оптимизации (R5, поправка
 *     №4), а не на список прилагательных (список в русском не закрывается,
 *     союз закрывается).
...
final class CommunityGuard
{
    ...
    /**
     * ADR-177 §2, R4 добавлена поправкой ADR-178 №2, R5 — поправкой №4 —
     * сравнительно-оценочная форма, ключуется на союз/корень/оборот, НЕ на
     * список прилагательных. Пять детерминированных правил, срабатывает любое
     * (регулярки — дословно из ADR):
     ...
     *  - R5 (ADR-178, поправка №4) клауза-императив + маркер оптимизации по
     *    времени/моменту в одном предложении («Собирай ресурсы ночью — охрана
     *    спит»). Императив покрыть целиком нельзя (законные «Поставь Теплицу»,
     *    «Скрафти флягу» — ядро ответов новичку), поэтому разделительную работу
     *    несёт адъюнкт: время/момент — признак лайфхака, место/предусловие —
     *    признак законной инструкции («Дрон заряжается, пока ты на базе» и
     *    «Ночью на карте темнее» императива не имеют, проходят). 🔴 Стоп-правило
     *    ADR-178: после R5 новые правила формы не добавляются без замеренного
     *    класса утечки на живом чате — пример класса не образует.
     */
    private function isComparativeClaim(string $answer): bool
    {
        ...
        // R5 (ADR-178, поправка №4) — клауза-императив И маркер оптимизации по
        // времени/моменту в ОДНОМ предложении. Регулярки дословно из поправки.
        // Императив тут — защита ОПИСАТЕЛЬНЫХ фраз («Дрон заряжается, пока ты на
        // базе», «Ночью на карте темнее» императива не имеют и проходят), не
        // полнота: разделительную работу несёт адъюнкт (время/момент — лайфхак,
        // место/предусловие — законная инструкция), императив лишь отсекает
        // описание без совета. Обнаружение императива намеренно нестрогое
        // (окончания «й/и/ь» многозначны — «случай», «край», «ночь» тоже
        // подходят под них) — это ОК, потому что вторая клауза (маркер) делает
        // основную работу разделения.
        $hasImperativeClause = preg_match(
            '/(?:^|[,;:—-]\s*)(\p{L}{4,}(?:йте|ите|ьте|й|и|ь))(?![\p{L}])/u',
            $lower,
        ) === 1;
        $hasTimingMarker = preg_match(
            '/(?<![\p{L}])(ночью|днём|днем|утром|вечером|на\s+рассвете|в\s+сумерках|пока|перед|во\s+время)(?![\p{L}])/u',
            $lower,
        ) === 1;
        if ($hasImperativeClause && $hasTimingMarker) {
            return true;
        }
    ...
    /**
     * Story 73 — «потолок» теряет беглую гласную «о» в косвенных падежах
     * («потолка», не «потолока»): буквальный префикс «потолок» ломается на
     * шестой букве, падежные формы не ловились бы вовсе без стема «потол»
     * (см. `matchesStopWord()`). Единственное такое слово в `LEXICAL_STOPLIST` —
     * у остальных склонение просто ДОБАВЛЯЕТ буквы к концу, беглых гласных нет.
     */
    private const STEM_OVERRIDES = ['потолок' => 'потол'];
     ...
     *
     * 🔴 Story 73 — допуск на лишние буквы зависит от длины стема, не
     * фиксирован в одну букву. Фикс story 69 закрыл «место»/«стоит», но
     * заплатил пропуском падежных форм самого стоп-слова («потолка», «потолки»,
     * «порогов») — русское падежное окончание обычно 1-3 буквы, не умещается в
     * одну. КОРОТКИЕ корни (≤4 буквы: «сто», «три», «один» — story 69 доказала
     * именно на них случайное совпадение с посторонними словами) держат
     * строгий допуск в одну букву; более длинные и различимые слова (≥5 букв)
     * получают допуск до трёх — собрать случайную коллизию на 5+-буквенном
     * корне маловероятно, а «потолка»/«порогов» без этого не ловились вовсе.
     */
    private function matchesStopWord(string $lower, string $word): bool
    {
        ...
        $suffixTolerance = mb_strlen($stem) <= 4 ? '{0,1}' : '{0,3}';

        return preg_match(
            '/(?<![\p{L}])' . preg_quote($stem, '/') . '\p{L}' . $suffixTolerance . '(?![\p{L}])/u',
            $lower,
        ) === 1;
    }
