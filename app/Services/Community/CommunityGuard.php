...
final class CommunityGuard
{
    ...
    /**
     * Story 69: «дцать»/«десят» — единственные два слова в {@see NUMERAL_WORDS},
     * которые НИКОГДА не встречаются как отдельное слово (они инфикс-фрагменты
     * составных числительных: «два·дцать», «пять·десят»). Граница слова (см.
     * {@see matchesStopWord()}) их убила бы целиком — им оставлен старый
     * substring-матч. Это единственное исключение, состав списков не менялся.
     */
    private const SUBSTRING_FRAGMENT_WORDS = ['дцать', 'десят'];
    ...
    private function isComparativeClaim(string $answer): bool
    {
        ...
        //  - «а не» — безусловно, КРОМЕ связки уточнения («а не только» / «а не
        //    просто» / «а не лишь» — это не сопоставление двух вариантов, а
        //    уточнение объёма одного, «можно говорить, а не только драться»).
        if (preg_match('/(?<![\p{L}])а\s+не(?!\s+(только|просто|лишь)(?![\p{L}]))(?![\p{L}])/u', $lower) === 1) {
            return true;
        }
     ...
     * story 30 (хвост «…да?») — лечится тем же приёмом: граница слова + МАКСИМУМ
     * одна лишняя буква-окончание («сто»→«сто»/«стом» проходят, «стоит» с двумя
     * лишними буквами — нет). `SUBSTRING_FRAGMENT_WORDS` — единственное исключение
     * («дцать»/«десят»): они НИКОГДА не бывают отдельным словом, это инфикс-
     * фрагменты составных числительных, граница убила бы их целиком.
     */
    private function matchesStopWord(string $lower, string $word): bool
    {
        ...
        return preg_match('/(?<![\p{L}])' . preg_quote($word, '/') . '\p{L}?(?![\p{L}])/u', $lower) === 1;
    }
