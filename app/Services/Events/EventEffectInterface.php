...
/**
 * F7.2 — інтерфейс для всех 10 effect-стратегій.
 *
 * Принцип: **чисти компуторы** (pure-ish, можна mt_rand).
 * - НЕ читають DB.
 * - НЕ шлють Telegram.
 * - НЕ записують в модели.
 *
 * Усе I/O — на dispatcher'и (F7.3 EventTickHandler) и accumulator'и (F7.4
 * EffectAccumulator). Це робить effect-классы легко тестованими и дозволяесть
 * dispatcher'в батчити SQL.
 *
 * @see app/Services/Events/Effects/*  — реалізаціи
 * @see mmorpg-vault/lore/refactor/F7-Audit.md
 */
interface EventEffectInterface
{
    /**
     * Вычислити ефект для одного character'а на одномв tick'у.
     *
     * @param array<string, mixed> $character    row с `characters` (id, biome_id, health, tired, gold, level, ...)
     * @param array<string, mixed> $eventConfig  запис с WorldEvents::$events для цієи подіи
     *                                           (effect_kind + effect_params + duration + ...)
     * @param array<string, mixed> $activeEvent  row с `active_events` (start_time, end_time, status)
     ...
     *                                           - 'is_gathering' : bool
     *                                           - 'is_exploring' : bool
     *                                           - 'biome'        : row с biomes (danger_level, survival_difficulty, biome_type)
