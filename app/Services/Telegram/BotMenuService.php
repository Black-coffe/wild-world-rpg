...
class BotMenuService
{
    ...
    public static function actionLabel(string $key): string
    {
        return match ($key) {
            ...
            // Та же дверь, но в ТЕСНОМ ряду направлений (killswitch слайса OFF): подпись
            // физически не влезала рядом с «⬅️ Запад»/«➡️ Восток», и её срезали до эмодзи.
            // Ключ существует ровно затем, чтобы тексты, которые эту кнопку называют, могли
            // назвать её так, как игрок её ВИДИТ, а не так, как она подписана на других
            // экранах. При ON дверь переезжает в свой ряд и ключ становится не нужен.
            'actionsHubCompact' => '🧑‍🌾 🛠️',
            default             => '',
        };
    }
