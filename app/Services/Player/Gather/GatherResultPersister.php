 ...
 * текст «груз некуда везти» (а не молчание) — при `$foldCargoNote=true` (M10-ремонт)
 * эта строка тоже сворачивается в общее сообщение о добыче, не уходит отдельным
 * `sendMessage`. Килсвитч off / нет активного транспорта /
...
class GatherResultPersister
{
    ...
    private function writeCharacterResources(array $foundResources, array|\App\Entities\CharacterEntity $character, bool $foldCargoNote): ?string
    {
        ...
        if (! $this->hasBase($charId)) {
            ...
            // m1-ремонт: и эта ветка сворачивается в общее сообщение о добыче по тем же
            // правилам, что и успешная доставка (не только доставка) — иначе владелец
            // машины без базы получал бы второе сообщение на КАЖДУЮ добычу.
            $this->creditBackpack($foundResources, $character);
            $note = $this->composeNoBaseNote();
