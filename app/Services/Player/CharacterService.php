...
class CharacterService
{
    ...
    public function showCharacterInfo(int $chatId, array|\App\Entities\CharacterEntity $characterRow): ServerResponse
    {
        ...
        if ($finalGrid) {
            ...
            // Раскладка 2026-07-27 (фидбэк владельца): НИ ОДНОЙ одиночной кнопки в ряду +
            // порядок сверху вниз ПО ЧАСТОТЕ. «🧑‍🌾 Действия» — самая нагруженная кнопка игры
            // (контекстный вход к действиям на клетке), поэтому она в левом верхнем углу, а не
            // третьим рядом. Ряды не прописываются руками: набор кнопок плавает (фракция /
            // дейлики / хабы / подать — условные), поэтому плоский список пакуется
            // {@see self::packButtonRows()}. tripleAt=2 → при нечётном числе кнопок ряд из
            // трёх соберётся на самых КОРОТКИХ подписях (Экип/Аптечка/Страховка), а не на
            // длинных — иначе ряд переносится на узком экране.
            $personalFlat = [
