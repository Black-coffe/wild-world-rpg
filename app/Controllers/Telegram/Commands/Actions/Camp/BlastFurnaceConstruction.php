...
class BlastFurnaceConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        $building = $this->buildingModel->where('name_en', 'BlastFurnace')->first();
        if ($character['level'] < $building['level']) {
            ...
            return Request::sendMessage([
                'chat_id' => $this->callbackQuery->getMessage()->getChat()->getId(),
                'text'    => "Ваш уровень слишком низкий для строительства *🔥 Доменной печи*.",
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }
        ...
        // 5. Отображение информации о строительстве
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🧑‍🌾 Действия 🛠️', 'callback_data' => 'characterActions'],
        ...
        $text = "*🔥 Доменная печь!*\n\n"
            ...
            . "\n*Строительство займет:* ~40 минут!\n"
            . "\n*Описание:* Печь Необходима для плавки металла потребуется в инженерных сооружениях и для некоторых компонентов\n\n";
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $text .= "\nНедостаточно ресурсов для строительства *🔥 Доменной печи*.\n\n";
            if ($missingResourcesText) {
                $text .= "Недостающие ресурсы:\n" . $missingResourcesText . "\n";
            }
            if ($missingCraftedItemsText) {
                $text .= "Недостающие крафтовые предметы:\n" . $missingCraftedItemsText . "\n";
            }
        }
