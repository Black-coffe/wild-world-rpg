...
class BuildGymConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        $building = $this->buildingModel->where('name_en', 'Gym')->first();
        if ($character['level'] < $building['level']) {
            ...
            return Request::sendMessage([
                'chat_id' => $this->callbackQuery->getMessage()->getChat()->getId(),
                'text'    => "Ваш уровень слишком низкий для строительства *🥊 Спортзала*.",
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }
        ...
        $text = "*🥊 Спортзал*\n\n"
            ...
            . "\n*Описание:* Спортивный и тренировочный зал, наличие которого дает каждые 5 минут 0,01 к силе персонажа\n";

        if (!$workshopExists) {
            $text .= "\nДля строительства *🥊 Спортзала* необходимо наличие *Мастерской*.\n";
        }
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $missingText = "\nНедостаточно ресурсов для строительства *🥊 Спортзал*.\n\n";
            if ($missingResourcesText) {
                $missingText .= "Недостающие ресурсы:\n" . $missingResourcesText . "\n";
            }
            if ($missingCraftedItemsText) {
                $missingText .= "Недостающие крафтовые предметы:\n" . $missingCraftedItemsText . "\n";
            }
        }
        ...
        if (strlen($text . $missingText) > 1024) {
            // Разбиваем текст на несколько сообщений
            $textParts = $this->splitText($text . $missingText, 1024);
