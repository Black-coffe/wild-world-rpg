...
class StartBuildGreenhouseConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        $building = $this->buildingModel->where('name_en', 'Greenhouse')->first();
        if ($character['level'] < $building['level']) {
            ...
            return Request::sendMessage([
                'chat_id' => $this->callbackQuery->getMessage()->getChat()->getId(),
                'text'    => "Ваш уровень слишком низкий для строительства *🌱 Теплицы*.",
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $messageText = "Недостаточно ресурсов для строительства *🌱 Теплицы*.\n\n";
    ...
    {
        $craftTask = $this->taskModel->where('name', 'startBuildGreenhouse')->first();
        if (!$craftTask) {
            return $this->sendError('Задача "Теплица" не найдена в базе данных.');
        }
