...
class StartBuildRoboticsWorkshopConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        $building = $this->buildingModel->where('name_en', 'RoboticsWorkshop')->first();
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $messageText = "Недостаточно для строительства *🤖 Мастерская робототехники*.\n\n";
            if ($missingResourcesText) {
                $messageText .= "Ресурсы:\n" . $missingResourcesText . "\n";
            }
            if ($missingCraftedItemsText) {
                $messageText .= "Крафт предметы:\n" . $missingCraftedItemsText . "\n";
            }
    ...
    {
        $craftTask = $this->taskModel->where('name', 'startBuildRoboticsWorkshop')->first();
        if (!$craftTask) {
            return $this->sendError('Задача "Строительство мастерской робототехники!" не найдена в базе данных.');
        }
    ...
    private function notifyCraftStarted($character, $startTime, $endTime): ServerResponse
    {
        ...
        $text = "*Процесс строительства запущен*\n\n"
            . "*Ты строишь: 🤖 Мастерская робототехники!*\n\n"
