...
class StartBuildBlastFurnaceConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $messageText = "Недостаточно ресурсов для строительства *🔥 Доменная печь*.\n\n";
    ...
    {
        $craftTask = $this->taskModel->where('name', 'buildBlastFurnace')->first();
        if (!$craftTask) {
            return $this->sendError('Задача "Строительство доменной печи!" не найдена в базе данных.');
        }
