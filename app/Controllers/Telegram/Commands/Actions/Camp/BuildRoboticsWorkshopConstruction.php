...
class BuildRoboticsWorkshopConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        if ($workshopExists && empty($missingResources) && empty($missingCraftedItems)) {
            array_unshift($keyboard['inline_keyboard'], [['text' => '🛠️ Строить', 'callback_data' => 'genericStartBuild_RoboticsWorkshop']]);
        }

        $text = "*🤖 Мастерская робототехники*\n"
            . "Тебе нужны:\n\n"
            ...
            . "\n*Стройка:* ~100 минут!\n"
            . "\n*Описание:* Дает возможность строить различного вида роботов и автоматические установки/машины.\n";

        if (!$workshopExists) {
            $text .= "\nДля строительства *🤖 Мастерская робототехники* необходима *Мастерская*.\n";
        }
