...
class BuildWarehouseConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        if ($character['level'] < $building['level']) {
            ...
            return Request::sendMessage([
                'chat_id' => $this->callbackQuery->getMessage()->getChat()->getId(),
                'text'    => "Ваш уровень слишком низкий для строительства *🏚️ Склада*.",
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }
        ...
        $text = "*🏚️ Склад*\n\n"
            ...
            . "\n*Описание:* Дает возможность торговли Крафтовыми предметами на закрытом рынке, а также снимает ограничения лимитов ресурсов на базе\n\n";

        if (!$workshopExists) {
            $text .= "\nДля строительства *🏚️ Склада* необходимо наличие *Мастерской*.\n";
        }
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $text .= "\nНедостаточно ресурсов для строительства *🏚️ Склада*.\n\n";
