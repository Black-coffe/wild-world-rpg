...
class TeleportBeaconMoveAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        foreach ($beacons as $beacon) {
            ...
            // Формируем строчку описания маяка. Остаток телепортов раньше стоял хвостом
            // «ТП. 87» — это единственное место в игре, где заряд маяка вообще был виден,
            // и прочитать его было нельзя (вопрос игрока 18.08.2026).
            $lines[] = "👉 `X={$x} Y={$y}` · 🪸{$biomeName} · ⚡ осталось телепортов: *{$remainingUses}*";
        ...
        $textMessage = $introText . implode("\n", $lines)
            . "\n\n"
            . "_Каждое перемещение тратит один телепорт маяка. Когда телепорты кончатся, "
            . "маяк перестанет принимать — на его место придётся ставить новый._";
