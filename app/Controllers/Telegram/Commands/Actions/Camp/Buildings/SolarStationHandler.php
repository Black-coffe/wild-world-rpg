...
class SolarStationHandler extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        if (!$characterBuilding) {
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'У вас нет Солнечной станции на базе.',
            ]);
        }

        // Получаем информацию о самой постройке
        $buildingInfo = $this->buildingModel->find($buildingId);
        if (!$buildingInfo) {
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text'    => 'Информация о Солнечной станции не найдена.',
            ]);
        }

        // Путь к изображению (убедитесь, что путь корректен)
        $imagePath = base_url('uploads/telegram/camp/solar_power_station.jpg');

        // Формируем текстовое описание (аналогично другим классам)
        $text = sprintf(
            "🏗️ *%s*\n\n".
            "📅 *Дата постройки:* %s\n".
            "⏳ *Дата исчезновения:* %s\n".
            ...
            // Если у buildingInfo есть usage_count и usage_count у персонажа не null — считаем остаток, иначе "Без ограничений"
            ...
            // Переводим поле usage в человекопонятное описание (персональная/коллективная/общая)
