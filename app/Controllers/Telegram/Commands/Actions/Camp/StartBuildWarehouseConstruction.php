...
class StartBuildWarehouseConstruction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        if ($missingResources || $missingCraftedItems) {
            ...
            $messageText = "Недостаточно ресурсов для строительства *🏚️ Склада*.\n\n";
    ...
    {
        $craftTask = $this->taskModel->where('name', 'startBuildWarehouse')->first();
        if (!$craftTask) {
            return $this->sendError('Задача "Строительство склада!" не найдена в базе данных.');
        }
