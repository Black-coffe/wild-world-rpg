...
class MoveNewLocationToNorthWest
{
    ...
    public function handle(): ServerResponse
    {
        ...
        // Получение текущей локации персонажа и определение северо-западной ячейки
        ...
        if (!$northwestCell) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Северо-западная локация не найдена.']);
        }

        // Проверка, исследована ли северо-западная ячейка
        ...
        if (!$explored) {
            ...
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'Северо-западная локация не исследована! Невозможно переехать.',
                'parse_mode' => 'Markdown',
        ...
        $this->characterModel->update($character['id'], [
            ...
            'biome_id' => $northwestCell['biome_id'], // Добавляем ID биома новой локации
        ...
        $biome = $this->biomeModel->find($northwestCell['biome_id']);
        if (!$biome) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Информация о биоме не найдена.']);
        }

        // Отправка сообщения об успешном переезде
        $text = "🚚 *Путешественник, ликуй! Ты успешно добрался на Северо-запад*.\n\n"
            . "🔲 *Твоя игровая ячейка: №*" . $northwestCell['cell_number'] . "\n\n"
            . "🧭 *Координаты: X* " . $northwestCell['coordinate_x'] . "* | Y* " . $northwestCell['coordinate_y'] . "\n\n"
