...
class QuestStartExplore30Cells extends BaseAction
{
    public function handle(): ServerResponse
    {
        ...
        $quest = $questModel->where('title_en', 'Explore30Cells')->first();

        if (!$quest) {
            Request::answerCallbackQuery(['callback_query_id' => $this->callbackQuery->getId()]);
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'Квест *Изучить 30 ячеек* не найден.',
                'parse_mode' => 'Markdown',
            ]);
        }
        ...
        if ($activeQuestSteps && in_array($quest['id'], array_column($activeQuestSteps, 'quest_id'))) {
            ...
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => "Квест *Изучить 30 ячеек* уже был запущен.\nСмотрите его в разделе:\n*'🚀 Активные квесты'*\nили в разделе\n*'📅 Доступные квесты'*.\n",
                'parse_mode' => 'Markdown',
            ]);
        }
        ...
        try {
            ...
            $questStepModel->insert([
                ...
                'step_order' => 1,
                'description' => 'Начало квеста на изучение 30 ячеек',
                'is_completed' => false
            ]);
        } catch (\Exception $e) {
