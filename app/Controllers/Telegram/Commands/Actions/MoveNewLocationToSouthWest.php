...
class MoveNewLocationToSouthWest
{
    ...
    public function handle(): ServerResponse
    {
        ...
        // Получение текущей локации персонажа и определение юго-западной ячейки
        ...
        $southwestCell = $this->mapModel
            ->where('coordinate_x', $currentCell['coordinate_x'] - 1)
            ->where('coordinate_y', $currentCell['coordinate_y'])->first();

        if (!$southwestCell) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Юго-западная локация не найдена.']);
        }

        // Проверка, исследована ли юго-западная ячейка
        $explored = $this->exploredCellsModel->where('character_id', $character['id'])->where('map_cell_id', $southwestCell['id'])->first();
        if (!$explored) {
            $keyboard = [
                'inline_keyboard' => [
                    [
            ...
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'Юго-запад - локация не исследована! Невозможно переехать.',
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }

        // Перемещение персонажа в юго-западную ячейку
        $this->characterModel->update($character['id'], [
            'cell_number' => $southwestCell['cell_number'],
            'biome_id' => $southwestCell['biome_id'], // Добавляем ID биома новой локации
        ...
        $biome = $this->biomeModel->find($southwestCell['biome_id']);
        if (!$biome) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Информация о биоме не найдена.']);
        }

        // Отправка сообщения об успешном переезде
        $text = "🚚 *Путешественник, ликуй! Ты успешно добрался на Юго-запад*.\n\n"
            . "🔲 *Твоя игровая ячейка: №*" . $southwestCell['cell_number'] . "\n\n"
            . "🧭 *Координаты: X* " . $southwestCell['coordinate_x'] . "* | Y* " . $southwestCell['coordinate_y'] . "\n\n"
