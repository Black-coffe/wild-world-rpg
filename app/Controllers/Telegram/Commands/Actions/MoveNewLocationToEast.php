...
class MoveNewLocationToEast
{
    ...
    public function handle(): ServerResponse
    {
        ...
        // Получение текущей локации персонажа и определение восточной ячейки
        ...
        $eastCell = $this->mapModel
            ->where('coordinate_x', $currentCell['coordinate_x'] + 1)
            ->where('coordinate_y', $currentCell['coordinate_y'])
            ->first();
        ...
        // Проверка, исследована ли восточная ячейка
        $explored = $this->exploredCellsModel->where('character_id', $character['id'])->where('map_cell_id', $eastCell['id'])->first();
        if (!$explored) {
            $keyboard = [
                'inline_keyboard' => [
                    [
        ...
        // Перемещение персонажа в Восточную ячейку
        $updateData = [
            'cell_number' => $eastCell['cell_number'],
            'biome_id' => $eastCell['biome_id'], // Добавляем ID биома новой локации
            ...
            'intellect' => max($character['intellect'] - 0.01, 0)
        ];

        $this->characterModel->update($character['id'], $updateData);

        // Получение информации о биоме
        $biome = $this->biomeModel->find($eastCell['biome_id']);
        if (!$biome) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Информация о биоме не найдена.']);
        }

        // Отправка сообщения об успешном переезде с информацией о биоме
        $text = "🚚 *Путешественник, ликуй! Ты успешно добрался на Восток*.\n\n"
            . "🔲 *Твоя игровая ячейка: №*" . $eastCell['cell_number'] . "\n\n"
            . "🧭 *Координаты: X* " . $eastCell['coordinate_x'] . "* | Y* " . $eastCell['coordinate_y'] . "\n\n"
