...
class ExploreAreaTipsAction extends BaseAction
{
    ...
    private function formatExplorationResultMessage($surroundingCells) {
        ...
        $message .= "\n🌫️ *Туман войны.* В игре ты видишь только круг вокруг себя — 1 клетку во все стороны (включая по диагонали). Дальше — туман.\n\n"
            . "👣 *Двигаясь, ты сам открываешь местность:* каждый шаг подсвечивает клетку, в которую ты пришёл, и её 8 соседей — и они остаются на твоей карте навсегда. Никакого отдельного «исследования» больше нет — иди и смотри.\n\n"
            . "🚜 *В дальний путь — «Дальний поход»:* задаёшь направление и сколько клеток пройти; идёшь сам (≈1 клетка в минуту), карта обновляется по дороге, отчёт по прибытии. Поход прервётся, если впереди что-то потребует решения (встреча, рана, чужой лагерь, край мира).\n\n"
