...
class PainReliefPowerCraftActionStart extends BaseAction
{
    ...
    private function startCraftingProcess($character, $userId): ServerResponse
    {
        ...
        if (!$craftTask) {
            return $this->sendError('Задача "Крафт Обезболивающего порошка" не найдена в базе данных.');
        }
    ...
    private function notifyCraftStarted($character, $startTime, $endTime): ServerResponse
    {
        ...
        $imagePath = base_url('uploads/telegram/craft/analgesic_powder.jpg'); // Исправлен путь к изображению
        Request::answerCallbackQuery(['callback_query_id' => $this->callbackQuery->getId()]);
