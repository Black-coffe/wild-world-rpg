...
class RegeneratorCraftActionStart extends BaseAction
{
    ...
    private function startCraftingProcess($character, $userId): ServerResponse
    {
        ...
        if (!$craftTask) {
            return $this->sendError('Задача "Крафт Успокоительного" не найдена в базе данных.');
        }
    ...
    private function notifyCraftStarted($character, $startTime, $endTime): ServerResponse
    {
        ...
        $imagePath = base_url('uploads/telegram/craft/health_and_strength_regenerator.jpg'); // Исправлен путь к изображению
        Request::answerCallbackQuery(['callback_query_id' => $this->callbackQuery->getId()]);
