...
class StonePickaxeCraftActionStart extends BaseAction
{
    ...
    private function startCraftingProcess($character, $userId): ServerResponse
    {
        ...
        if (!$craftTask) {
            return $this->sendError('Задача "Крафт ⛏️ Каменной кирки" не найдена в базе данных.');
        }
    ...
    private function notifyCraftStarted($character, $startTime, $endTime): ServerResponse
    {
        ...
        $text = "*Процесс крафта запущен*\n\n"
            . "*Ты создаешь: ⛏️ Каменную кирку!*\n\n"
            . "__*Время крафта: " . $minutes . " минут.*__ ⏱️\n\n"
            . "*О готовности ты узнаешь в сообщении.* 🎁\n\n";

        $imagePath = base_url('uploads/telegram/craft/create-an-image-of-an-ancient-stone-pickaxe.jpg'); // Исправлен путь к изображению
        Request::answerCallbackQuery(['callback_query_id' => $this->callbackQuery->getId()]);
