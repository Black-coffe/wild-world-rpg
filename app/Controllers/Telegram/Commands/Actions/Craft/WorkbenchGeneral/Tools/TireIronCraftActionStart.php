...
/**
 * Класс, запускающий количественный крафт "Монтировки" (TireIron).
 */
class TireIronCraftActionStart extends BaseAction
{
    protected $taskModel;
    ...
    /**
     * Количество монтировок, извлекаемое из callback_data. По умолчанию 1.
     */
    ...
    public function __construct($callbackQuery)
    {
        ...
        // Пример: callback_data="craftTireIron_10"
        // => ["craftTireIron","10"] => quantity=10
    ...
    public function handle(): ServerResponse
    {
        ...
        if (!$this->checkAndDeductResources($character['id'], $this->quantity)) {
            return $this->sendError("Недостаточно ресурсов для крафта {$this->quantity} монтировок.");
        }
    ...
    /**
     * Списываем ресурсы (Железная руда×54) × $qty.
     */
    ...
    /**
     * Создаём запись в character_tasks, записывая quantity в task_settings.
     * Рассчитываем общее время: базовое_время×qty.
     */
    private function startCraftingProcess(array $character, int $userId, int $qty): ServerResponse
    {
        // Ищем задачу craftTireIron
        $craftTask = $this->taskModel->where('name', 'craftTireIron')->first();
        if (!$craftTask) {
            return $this->sendError('Задача "Крафт Монтировки" (craftTireIron) не найдена в базе.');
        }

        // Проверяем, нет ли уже активной такой же задачи
        ...
        if ($activeTask) {
            return $this->sendError(
                "У тебя уже идёт крафт монтировок! Дождись окончания или прерви," .
                " но ресурсы не вернутся."
            );
        }
        ...
        // Запомним quantity в task_settings
        $taskSettings = [
            'quantity' => $qty
        ];
    ...
    /**
     * Рассчитываем базовое время (на 1 шт.) через формулу атрибутов, затем умножим на qty.
     */
    ...
    /**
     * Уведомляем: "Создаёшь X монтировок, прерывание потеря ресурсов, итого время Y".
     */
    private function notifyCraftStarted(array $character, \DateTime $startTime, \DateTime $endTime, int $qty): ServerResponse
    {
        ...
        $text = "*Процесс крафта запущен*\n\n"
            . "Ты создаёшь: *🪛 Монтировки* x{$qty} шт.\n\n"
    ...
    /**
     * Преобразуем totalMinutes в строку "X дней Y часов Z минут".
     */
    ...
    /**
     * Подбираем форму слова в русском языке (день/дня/дней).
     */
