...
/**
 * Класс запуска задачи крафта "Трубчатый пистолет" (PipeGun).
 ...
 */
class StartCraftPipeGun2Action extends BaseAction
{
    ...
    public function __construct($callbackQuery)
    {
        ...
        // Разбираем callback_data вида "startCraftPipeGun_10" => quantity=10
    ...
    public function handle(): ServerResponse
    {
        ...
        // 2) Проверяем, есть ли база
        $base = $this->claimModel->where('character_id', $character['id'])->first();
        if (!$base) {
            return $this->sendError($chatId, 'У вас нет базы, крафт невозможен.');
        }

        // 3) Ищем «PipeGun» в таблице weapons, чтобы взять динамические req (required_strength/level)
        $weapon = $this->weaponModel->getByEnglishName('PipeGun');
        if (!$weapon) {
            return $this->sendError($chatId, "«PipeGun» не найден в weapons!");
        }

        // Берём силу/уровень из weapon, но гарантируем минимум
        $requiredStrength = max(0, (int)$weapon['required_strength']);
        ...
        // 5) Допустим, золота нужно 600 за штуку
        $requiredGold = 600;
        ...
        // 6) Крафтовые предметы (пример)
        // Придумайте логично под PipeGun: "Металл фрагменты" => 4, "Складной нож" => 1, "Ткань" => 4
        $craftedNeed = [
            'Металл фрагменты' => 4,
            'Складной нож'     => 1,
            'Ткань'            => 4,
        ...
        // 7) Сырьевые ресурсы (пример)
        // "Древесина" => 2, "Кожа животных" => 2, "Смола деревьев" => 2
        $rawNeeded = [
        ...
        // 11) Ищем/создаём задачу craftPipeGun
        $taskRow = $this->taskModel->where('name', 'craftPipeGun')->first();
        if (!$taskRow) {
            $newTaskId = $this->taskModel->insert([
                'name'                       => 'craftPipeGun',
                'name_rus'                   => 'Крафт Трубчатого пистолета',
                'description'                => 'Создание самодельного Pipe Gun',
                'min_duration'               => 5,
                'max_duration'               => 15,
                'type'                       => 'craft',
                'difficulty_level'           => 3,
            ...
            $taskRow = ['id' => $newTaskId, 'name' => 'craftPipeGun'];
        }

        // 12) Проверяем, не крафтим ли уже PipeGun
        ...
        if ($activeTask) {
            return $this->sendError(
                $chatId,
                'У вас уже идёт крафт «Трубчатого пистолета»! Дождитесь завершения старого.'
            );
        }

        // 13) Определяем время крафта (допустим 8 мин за 1 шт.)
        $timeForOne = 8;
        ...
        $text = "*Начат крафт {$this->quantity} шт. «Трубчатого пистолета»!*\n"
    ...
    /**
     * Удобный метод для ответа об ошибке и закрытия "часиков".
     */
