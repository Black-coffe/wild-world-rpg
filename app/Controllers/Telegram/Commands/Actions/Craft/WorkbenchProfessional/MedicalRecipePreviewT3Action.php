 ...
 *
 * Callback prefix: `craftPreviewT3Medical_<RecipeKey>` (см. CallbackRoutes.prefixRoutes).
 * Recipe key извлекается из callback suffix → lookup в Config\CraftRecipes →
 * рендер preview: heal-эффект (из GameSettings) + multi-use + recipe requirements
 * + кнопка «🛠 Скрафтить».
 *
 * Зеркало `ArmorRecipePreviewT3Action` (S18) — но вместо outfit-статов показывает
 * heal-эффект из GameSettings (medical.<key>.heal_health/.heal_tired) и
 * durability_count (кол-во применений). CraftRecipes — single source of truth.
 */
class MedicalRecipePreviewT3Action extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        // Crafted item (drug) — для durability (кол-во применений) и name_rus.
        ...
        // Heal-эффект из GameSettings (admin-tunable).
        ...
        $usesRaw    = is_array($craftedItem) ? ($craftedItem['durability_count'] ?? 1) : 1;
        $uses       = max(1, $this->intFromMixed($usesRaw));
        ...
        if (!empty($effectParts)) {
            $perUse = $uses > 1 ? ' за применение' : '';
            $caption .= "_Эффект: " . implode(', ', $effectParts) . "{$perUse}_\n";
        }
        if ($uses > 1) {
            $caption .= "_Многоразовый: {$uses} применения_\n";
        }
        ...
        $rows[] = [
            ['text' => '⬅️ Назад к медицине', 'callback_data' => 'craftMedicalT3Select'],
    ...
    /**
     * Callback formatting: `craftPreviewT3Medical_<RecipeKey>` → '<RecipeKey>'.
     */
    private function extractRecipeKey(): string
    {
        $data   = (string) $this->callbackQuery->getData();
        $prefix = 'craftPreviewT3Medical_';
    ...
    /**
     * name_eng → setting-key segment (SyntheticMedicine → synthetic_medicine).
     */
