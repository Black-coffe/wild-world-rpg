...
use App\Models\CharacterResourceModel; // Модель для доступа к ресурсам персонажа
...
class StrengthElixirCraftActionStart extends BaseAction
{
    ...
    private function checkAndDeductResources($characterId): bool
    {
        ...
        // Списание ресурсов
        foreach ($requiredResources as $resourceName => $requiredAmount) {
            $this->characterResourceModel->deductResource($resourceName, $characterId, $requiredAmount);
        }
    ...
    private function notifyCraftStarted($character, $startTime, $endTime): ServerResponse
    {
        ...
        $text = "*Процесс крафта запущен*\n\n"
            . "*Ты создаешь: 🧪 Укрепляющий эликсир!*\n\n"
