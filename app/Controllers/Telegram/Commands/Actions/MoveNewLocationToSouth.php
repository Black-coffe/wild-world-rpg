...
class MoveNewLocationToSouth
{
    ...
    protected $playerDetectionService; // PlayerDetectionService
    ...
    {
        $logFilePath = WRITEPATH . 'logs/worker_log.txt';
        file_put_contents($logFilePath, "Start class MoveNewLocationToSouth method handle: " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
        ...
        // Получение текущей локации персонажа и определение южной ячейки
        ...
        $southCell = $this->mapModel->where('coordinate_x', $currentCell['coordinate_x'])
            ->where('coordinate_y', $currentCell['coordinate_y'] + 1)->first();

        if (!$southCell) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Южная локация не найдена.']);
        }

        // Проверка, исследована ли южная ячейка
        $explored = $this->exploredCellsModel->where('character_id', $character['id'])
            ->where('map_cell_id', $southCell['id'])->first();
        if (!$explored) {
            $keyboard = [
                'inline_keyboard' => [
                    [
            ...
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'Южная локация не исследована! Невозможно переехать.',
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }

        // Перемещение персонажа в южную ячейку
        $this->characterModel->update($character['id'], [
            'cell_number' => $southCell['cell_number'],
            'biome_id' => $southCell['biome_id'], // Добавляем ID биома новой локации
        ...
        $biome = $this->biomeModel->find($southCell['biome_id']);
        if (!$biome) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Информация о биоме не найдена.']);
        }

        // Отправка сообщения об успешном переезде
        $text = "🚚 *Путешественник, ликуй! Ты успешно добрался на Юг*.\n\n"
            . "🔲 *Твоя игровая ячейка: №*" . $southCell['cell_number'] . "\n\n"
            . "🧭 *Координаты: X* " . $southCell['coordinate_x'] . "* | Y* " . $southCell['coordinate_y'] . "\n\n"
