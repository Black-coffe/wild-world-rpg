...
/**
 * W5 (ADR-064) — preview-экран крафта DroneCombat. Зеркало
 * DroneRepairCraftInfoAction. Чек-лист ресурсов/компонентов/золота/гейта
 * Мастерской робототехники L4 с ✅/❌. Media-off safe.
 */
final class DroneCombatCraftInfoAction extends BaseAction
{
    private const RECIPE_KEY = 'DroneCombat';
    ...
    public function handle(): ServerResponse
    {
        ...
        if (! is_array($recipe)) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Рецепт боевого дрона не найден.']);
        }
        ...
        if ($hasAll) {
            ...
            $keyboard = ['inline_keyboard' => [[
                ['text' => '🛠️ Крафтить', 'callback_data' => 'genericCraft_DroneCombat_1'],
        ...
        // W5: art-tail ждёт `php spark images:generate --missing` — fallback на
        // image_in_progress (standard_craft_area.jpg) до генерации drone_combat.jpg.
    ...
    private function requiredWorkshopLevel(array $recipe): int
    {
        ...
        $raw    = $levels['RoboticsWorkshop'] ?? 4;
        return is_numeric($raw) ? max(1, (int) $raw) : 4;
    }
