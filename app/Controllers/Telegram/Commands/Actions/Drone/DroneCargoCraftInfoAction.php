 ...
 * Чек-лист ресурсов/компонентов/золота/гейта Мастерской робототехники с ✅/❌.
 * Media-off safe (caption самодостаточен).
 */
final class DroneCargoCraftInfoAction extends BaseAction
{
    private const RECIPE_KEY = 'DroneCargo';
    ...
    public function handle(): ServerResponse
    {
        ...
        if (! is_array($recipe)) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Рецепт карго-дрона не найден.']);
        }
        ...
        $text = "*{$icon} {$name}*\n\n";
        // Дрон — про расстояние, а не про место: вес рюкзака не ограничен вовсе
        // (`inventory.weight_cap.enabled=0`), и прежняя формулировка «переносит до 30 кг»
        // читалась как решение проблемы вместимости, которой нет (вопрос игрока 18.08.2026).
        $text .= "Грузовой квадрокоптер: забирает добычу с ЛЮБОЙ клетки и относит прямо на склад базы, пока ты сам далеко. "
            . "Он про расстояние, а не про место в рюкзаке — рюкзак по весу не ограничен, а вот возвращаться домой с каждой ходкой стоит времени. "
            . "За вылет уносит до 30 кг, потом заряжается на базе (~3 часа до полного). Стоя на базе, добычу можно сложить на склад и руками, без дрона.\n\n";
        ...
        if ($hasAll) {
            ...
            $keyboard = ['inline_keyboard' => [[
                ['text' => '🛠️ Крафтить', 'callback_data' => 'genericCraft_DroneCargo_1'],
    ...
    private function requiredWorkshopLevel(array $recipe): int
    {
        ...
        $raw    = $levels['RoboticsWorkshop'] ?? 2;
        return is_numeric($raw) ? max(1, (int) $raw) : 2;
    }
