...
class CargoDroneSelectAction extends BaseAction
{
    ...
    public function handle(): ServerResponse
    {
        ...
        $text .= "Или жми *«🤖 Автовывоз»*: дрон сам наберёт самое ценное по убыванию редкости, "
            . "не трогая еду, воду и семена.\n\n";
        ...
        // Автовывоз — один тап вместо восьми отправок подряд (просьба игрока 18.08.2026).
        // Показываем только когда есть что везти: кнопка на пустом рюкзаке была бы обещанием
        // действия, которое ничего не сделает.
        if (! empty($buttons)) {
            $buttons[] = [
                ['text' => '🤖 Автовывоз (самое ценное)', 'callback_data' => "cargoDroneAuto_{$logId}"],
            ];
        }
