...
class MoveNewLocationToSouthEast
{
    ...
    public function handle(): ServerResponse
    {
        ...
        // Получение текущей локации персонажа и определение юго-восточной ячейки
        ...
        $southeastCell = $this->mapModel
            ->where('coordinate_x', $currentCell['coordinate_x'] + 1)
            ->where('coordinate_y', $currentCell['coordinate_y'] + 1)->first();

        if (!$southeastCell) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Юго-восточная локация не найдена.']);
        }

        // Проверка, исследована ли юго-восточная ячейка
        $explored = $this->exploredCellsModel->where('character_id', $character['id'])->where('map_cell_id', $southeastCell['id'])->first();
        if (!$explored) {
            $keyboard = [
                'inline_keyboard' => [
                    [
            ...
            return Request::sendMessage([
                'chat_id' => $chatId,
                'text' => 'Юго-восток - локация не исследована! Невозможно переехать.',
                'parse_mode' => 'Markdown',
                'reply_markup' => json_encode($keyboard),
            ]);
        }

        // Перемещение персонажа в юго-восточную ячейку
        $this->characterModel->update($character['id'], [
            'cell_number' => $southeastCell['cell_number'],
            'biome_id' => $southeastCell['biome_id'], // Добавляем ID биома новой локации
        ...
        $biome = $this->biomeModel->find($southeastCell['biome_id']);
        if (!$biome) {
            return Request::sendMessage(['chat_id' => $chatId, 'text' => 'Информация о биоме не найдена.']);
        }

        // Отправка сообщения об успешном переезде
        $text = "🚚 *Путешественник, ликуй! Ты успешно добрался на Юго-восток*.\n\n"
            . "🔲 *Твоя игровая ячейка: №*" . $southeastCell['cell_number'] . "\n\n"
            . "🧭 *Координаты: X* " . $southeastCell['coordinate_x'] . "* | Y* " . $southeastCell['coordinate_y'] . "\n\n"
