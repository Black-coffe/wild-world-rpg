...
class Front extends BaseController
{
    ...
    private function metaDescription(string $raw, int $limit = 160): string
    {
        ...
        //
        // 🔴 ТОЛЬКО preg с флагом /u, НЕ rtrim со списком символов. `rtrim` трактует список
        // как набор БАЙТОВ, а в нём есть многобайтные тире (`—` = E2 80 94, `–` = E2 80 93).
        // Из-за этого он отгрызал байт 0x80 у последней кириллической буквы — например у «р»
        // (U+0440 = D1 80) — оставляя висячий D1. Строка становилась невалидным UTF-8,
        // `json_encode` всего графа Schema.org возвращал false, и страница уходила в прод с
        // ПУСТЫМ `<script type="application/ld+json"></script>`. Поймано закрывающим прогоном
        // на `/wild-world-pervyj-chas-novichka` — единственной странице из 139, где так совпало.
        $trimmed = preg_replace('~[\s.,;:!?—–-]+$~u', '', $cut);
        $cut     = is_string($trimmed) ? $trimmed : $cut;
