...
class CraftCompletionBasicMedKitHandler extends Controller
{
    ...
    /**
     * Метод, вызываемый по окончании крафта (через CRON или другой планировщик).
     *
     * @param array $task Данные о задаче (из character_tasks), включая task_settings.
     */
    public function handle($task)
    {
        ...
        // 2. Ищем предмет (BasicMedKit → англ. name_eng='FirstAidKit')
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'FirstAidKit')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item (FirstAidKit) not found in the database.');
            return;
        }

        // 3. Считываем quantity из task_settings (по умолчанию 1)
        ...
        // 5. Прокачиваем персонажу атрибуты (пример: +0.05 ловкости и интеллекта)
    ...
    /**
     * Извлекает "quantity" из task_settings (JSON), если есть, иначе возвращает 1.
     */
    ...
    /**
     * Сообщаем игроку, сколько шт. предмета создано и сколько теперь всего у него.
     */
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $quantityAdded)
    {
        ...
        // Русское название (если есть в базе)
        $itemNameRus = $craftedItem['name_rus'] ?? "Базовая Аптечка";
        ...
        // Кнопка "Крафтить ещё 1" (или любой другой кол-во)
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔄 Крафтить еще', 'callback_data' => 'craftBasicMedKit_1'],
        ...
        // Пытаемся ответить (если ещё актуально)
        try {
