...
class TremorHandler {
    ...
    public function process() {
        ...
        if (!$eventInfo) {
            return; // Событие "Tremor" не найдено
        }
        ...
        $activeEvent = $this->activeEventModel
            ...
            ->first();
        if (!$activeEvent) {
            return; // Активное событие "Tremor" не найдено
        }
            ...
            foreach ($tasksInWork as $taskInWork) {
                ...
                if (in_array($task['name'], ['Gather', 'ExploreTheArea'])) {
                    ...
                    if (rand(1, 5) === 3) {
                        ...
                        // Логирование эффектов события
                        $this->logTremorEffects($character['id'], $eventInfo['id'], $finalDamage, $newHealth);

                        // Добавить уведомление пользователя о полученном уроне
                        $message .= "⚠️ *Внимание*, во время события: ➡️ *Подземные толчки.*\n\n";
                        ...
                        $imagePath = base_url('uploads/telegram/earthquake.png'); // Укажите актуальный путь к изображению
    ...
    protected function logTremorEffects($characterId, $eventId, $damage, $newHealth) {
