...
class EventActivationHandler
{
    ...
    public function process() {
        ...
        if ($this->countActiveEvents() >= 1) {
            // Если уже есть 1 и более активных событий, новое событие не запускаем
            return;
        }
    ...
    protected function countActiveEvents() {
        // Подсчитываем количество активных событий
    ...
    protected function initializeTimeBoundaries() {
        // Установим воскресенье как начало недели (0 - воскресенье, 6 - суббота)
        $dayOfWeek = $this->now->getDayOfWeek();
        $daysUntilStartOfWeek = $dayOfWeek % 7; // Для воскресенья начало недели уже наступило
        $daysFromEndOfWeek = 6 - $dayOfWeek; // До конца недели от субботы

        // Используем subDays() и addDays() для корректировки текущей даты
        $this->startOfWeek = $this->now->subDays($daysUntilStartOfWeek)->setTime(0, 0, 0);
        $this->endOfWeek = $this->now->addDays($daysFromEndOfWeek)->setTime(23, 59, 59);
    }
    ...
    protected function isRecentActivationTooClose() {
        ...
        if ($lastActiveEvent) {
            ...
            // Генерируем случайное число минут от 540 до 1240
        ...
        // Если событий не было, считаем, что проверка на временной интервал не требуется, можно активировать событие
        return true;
    }
