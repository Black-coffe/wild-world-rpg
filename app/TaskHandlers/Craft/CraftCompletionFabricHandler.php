...
class CraftCompletionFabricHandler extends Controller
{
    ...
    public function handle($task)
    {
        ...
        // 2) Находим предмет (Fabric)
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'Fabric')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item "Fabric" not found in the database.');
            return;
        }
    ...
    /**
     * Извлекает 'quantity' из task_settings (JSON). По умолчанию 1.
     */
    ...
    /**
     * Отправляем уведомление, сколько «Ткани» добавлено и сколько всего теперь.
     */
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $quantityAdded)
    {
        ...
        $itemNameRus = $craftedItem['name_rus'] ?? "Ткань";

        $text = "📌 Вы успешно скрафтили:\n\n"
            . "🧵 *{$itemNameRus}* x{$quantityAdded} шт.\n\n"
            . "Теперь у вас *{$totalNow} шт.*\n\n"
        ...
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔄 Крафтить ещё', 'callback_data' => 'craftFabric_1'],
