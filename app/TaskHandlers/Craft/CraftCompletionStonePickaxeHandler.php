...
class CraftCompletionStonePickaxeHandler extends Controller
{
    ...
    public function handle($task)
    {
        ...
        // 2) Находим предмет (name_eng='StonePickaxe')
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'StonePickaxe')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item "StonePickaxe" not found in the database.');
            return;
        }
    ...
    /**
     * Извлекаем 'quantity' из поля task_settings (JSON). Если нет или нечисловое — 1.
     */
    ...
    /**
     * Уведомляем игрока об итоговом количестве кирок.
     */
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $quantityAdded)
    {
        ...
        $itemNameRus = $craftedItem['name_rus'] ?? "Каменная кирка";

        $text = "📌 *Крафт завершён!*\n\n"
            . "Ты создал: ⛏️ *{$itemNameRus}* x{$quantityAdded} шт.\n\n"
        ...
        $keyboard = [
            'inline_keyboard' => [
                [
                    // Можно указать callback_data='craftStonePickaxe_1', чтобы сразу крафтить ещё 1 шт.
                    ['text' => '🔄 Крафтить еще', 'callback_data' => 'craftStonePickaxe_1'],
