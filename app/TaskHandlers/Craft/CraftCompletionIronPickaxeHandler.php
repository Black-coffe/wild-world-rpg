...
class CraftCompletionIronPickaxeHandler extends Controller
{
    ...
    public function handle($task)
    {
        ...
        // 2. Находим предмет 'IronPickaxe'
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'IronPickaxe')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item "IronPickaxe" not found in the database.');
            return;
        }
    ...
    /**
     * Уведомляем игрока о том, сколько кирок теперь у него.
     */
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $quantityAdded)
    {
        ...
        $updatedLog = $this->craftedItemsLogModel
            ->where([
        ...
        $itemNameRus = $craftedItem['name_rus'] ?? "Железная кирка";
        ...
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔄 Крафтить еще', 'callback_data' => 'craftIronPickaxe_1'],
        ...
        // Пытаемся ответить на callbackQuery (если актуально)
        try {
