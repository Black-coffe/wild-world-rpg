...
class CraftCompletionRegeneratorHandler extends Controller
{
    ...
    /**
     * Основной метод, вызываемый при завершении задачи (через CRON или аналог).
     *
     * @param array $task Строка из таблицы character_tasks, в т.ч. с полем "task_settings".
     */
    public function handle($task)
    {
        // 1. Закрываем задачу (completed)
        $this->characterTaskModel->update($task['id'], ['status' => 'completed']);

        // 2. Ищем предмет "Regenerator" (name_eng='Regenerator')
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'Regenerator')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item "Regenerator" not found in the database.');
            return;
        }
        ...
        if ($existingLog) {
            ...
            // Создаём новую запись, если предмет не был создан ранее
    ...
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $quantityAdded)
    {
        ...
        // Узнаём, сколько теперь всего у игрока
        ...
        $itemNameRus = $craftedItem['name_rus'] ?? "Регенератор";
        ...
        // Кнопка "Крафтить ещё" указывает на колбэк "craftRegenerator_1"
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔄 Крафтить еще', 'callback_data' => 'craftRegenerator_1'],
        ...
        // Попытка ответить на callbackQuery (может уже устареть, если CRON)
        try {
