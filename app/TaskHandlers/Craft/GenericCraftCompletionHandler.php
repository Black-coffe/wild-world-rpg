...
class GenericCraftCompletionHandler extends BaseTaskHandler
{
    ...
    public function handle(array $task = []): void
    {
        ...
        // Упрочнение 2026-07-27: оба ключа читались БЕЗ `?? 0`, хотя три остальных
        // чтения бонусов в этом же файле (weapon/outfit/resource-ветки) застрахованы.
        // Бонус — НЕ обязательное поле рецепта: `agility_bonus` отсутствует у 2 из 100
        // рецептов, `intellect_bonus` — у 15.
        //
        // Сегодня ни один из них сюда не доходит: до этого блока добираются только
        // рецепты с `output_type='crafted_item'` (73 из 100 — остальные выходят по
        // `return` выше), и у всех 73 оба ключа на месте. То есть это защита на будущее:
        // новый crafted_item-рецепт без бонус-ключа ЛИБО смена output_type у
        // существующего роняли бы ЗАВЕРШЕНИЕ крафта здесь — уже ПОСЛЕ выдачи предмета и
        // ДО уведомления (шаг 7), а Worker помечает задачу completed ещё до handle()
        // (Worker.php:91-99, исключение глотается catch на 117), поэтому повтора нет:
