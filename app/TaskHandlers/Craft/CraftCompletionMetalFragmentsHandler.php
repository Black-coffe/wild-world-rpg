...
class CraftCompletionMetalFragmentsHandler extends Controller
{
    ...
    public function handle($task)
    {
        ...
        // 2) Ищем информацию о предмете (англ. name_eng = "metalFragments")
        $craftedItem = $this->craftedItemsModel->where('name_eng', 'metalFragments')->first();
        if (!$craftedItem) {
            log_message('error', 'Crafted item "metalFragments" not found in the database.');
            return;
        }

        // 3) Извлекаем количество из task_settings (quantity), по умолчанию =1
        ...
        // 5) Прокачиваем персонажа (пример: +0.05 ловкости, +0.05 интеллекта)
    ...
    /**
     * Отправляем уведомление игроку о завершении крафта.
     */
    private function notifyUser(int $telegramUserId, array $craftedItem, int $characterId, int $qtyAdded)
    {
        ...
        // Смотрим текущее общее количество
        $existingLog = $this->craftedItemsLogModel->where([
        ...
        // Русское имя предмета
        $itemNameRus = $craftedItem['name_rus'] ?? 'Металл фрагменты';
        ...
        // Кнопка "Крафтить еще" + "🎒 Инвентарь"
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🔄 Крафтить еще', 'callback_data' => 'craftMetalFragments_1'],
