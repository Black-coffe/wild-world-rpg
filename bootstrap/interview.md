...
13. Gate model policy: `scripts/top-model.sh --explain` has already read the plan - Fable 5.1 holds the gate where the subscription carries it inside its limits (Max, premium seats), Opus 5.5 where it would bill to credits (Pro, standard seats, API). The Queen and the workers run on Opus 5.5 either way (ADR-012). Confirm, or pin deliberately (sets TOP_MODEL; `auto` is the default and the right answer for almost everyone).
