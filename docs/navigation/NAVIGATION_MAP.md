...
| Переходов (рёбер-кнопок) | **862** | резолвленные кнопки экран→экран |
| Точек входа | **9** | reply-клавиатура (5) + слэш-команды (4) |
| **Мёртвые кнопки** | **72** | callback не резолвится ни в один маршрут |
| Тупики (экран без кнопок) | **24** | нет навигации дальше (часть легитимна) |
| Сироты (недостижимы) | **51** | ни одна кнопка/маршрут не ведёт сюда |
| Макс. глубина | **7** | кликов от точки входа до самого глубокого экрана |

> Из 72 «мёртвых» — 24 это `renew_building_`/`delete_building_` (подтверждённый баг),
> остаток — динамические callback (`'questStart'.$title`, `$callbackData`), которые
> резолвятся в рантайме, но требуют ручной проверки покрытия (см. §5).
...
flowchart TD
    ...
    CR --> GEN["Общий крафт T1"]
    CR --> STD["Стандарт крафт T2"]
    ...
    WB --> WB1["Верстак 1 (T1)"]
    WB --> WB3["Верстак 3 (T3)"]
    WB3 --> T3W["⚔️ Оружие T3"] & T3A["🛡 Броня T3"] & T3M["💊 Медицина T3"] & T3U["🔧 Утилиты T3"] & T3F["🎖 Фракционное"]
...
| Профи/Уникальный крафт | `proCraft/uniqueCraft` | `CraftService.php:40-46` (текст без кнопок) | меню крафта |
...
5. **`sleep(2)` в FortuneWheelAction:120** — блокирует PHP-процесс (600 игроков → очередь). Убрать.
6. **`RockPaperScissors` скрытый 40%-cap** + **GuessNumber «вдвое меньше» (реально 1.2×)** — врущие тексты, исправить/раскрыть.
...
23. **Хардкод-пороги золота → GameSettings**: `BuyCraftAction:45` (1000), `BuyResourceAction:40` (10), шансы `FortuneWheel` (конституция admin-tunable balance).
24. **PvP-поток** (RNG-fence: только детерминированные правки): защитник получает сообщение **без клавиатуры** (`AttackPlayerAction:323`); цель не уведомляется об обнаружении; нет «возобновить поход» после боя.
...
- **N2 — «Назад везде» (🟢, 1 сессия):** добавить навигацию в 22 превью + тупики §6.
  Тиражирование готового T3-паттерна, нулевой риск регрессии.
- **N3 — «Честные тексты» (🟢, 1 сессия):** caption=кнопки, MEDIA-OFF фиксы, `$`→`💰`,
  тексты игр, единое имя «Поход», `move`/`march` иконки, namespace-фиксы.
- **N4 — «Онбординг» (🟡, 1 сессия):** `/name`→ForceReply, прогресс-индикатор,
  убрать двойной /start-месседж, точка входа к фракции.
- **N5 — «Баланс в админку» (🟠, ADR):** золото-пороги, шансы игр → GameSettings.
- **N6 — «PvP-поток» (🟠, ADR, RNG-fence):** клавиатура защитнику, уведомление цели, resume-поход.
