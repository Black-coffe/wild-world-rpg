...
- **2026-09-15 · История 08 (волна 3): подтверждение апгрейда + baseline.** Триггер: воркер 03 сообщил в INTERFACES, что кнопку `confirm_upgrade_building_{id}` строит `BuildingUpgradeMessageFormatter::askPrompt()` — файл вне всех `## Files`; без суффикса клик по подтверждению уходит в легаси `resolve()`, и под Вышкой апгрейд со второй базы применился бы к первой (тот же класс бага, что чинит спека). Решение: история `multibase-picker-08` владеет форматтером подтверждения и — последней правкой кода — единственной чисткой `phpstan-baseline.neon` (см. дельту выше). `blocked_by` — вся волна 2. Отклонено: дописать в историю 03 (закрыта бы с дырой, и файл вне её `## Files`); оставить хвостом (ask 3 требует базу во всех кнопках цепочки).
